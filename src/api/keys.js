...
export async function getKeyByUid(uid) {
  ...
  const rows = await fetchJson(
    `${SUPABASE_URL}/rest/v1/keys?uid=eq.${encodeURIComponent(uid)}&select=*`,
    {
...
export async function loadAgentFromUID() {
  ...
  if (keyRow.agent_slug) {
    const agentData = await fetchJson(
      `${SUPABASE_URL}/rest/v1/agents?slug=eq.${encodeURIComponent(keyRow.agent_slug)}&select=*`,
      {
        headers: authHeaders(KEY)
      }
    );
...
export async function linkKeyToAgent(slug) {
  if (!state.uid) {
    throw new Error('Missing chip uid');
  }
  ...
  if (existingRow) {
    const patchRes = await fetch(
      `${SUPABASE_URL}/rest/v1/keys?uid=eq.${encodeURIComponent(state.uid)}`,
      {
        method: 'PATCH',
    ...
    if (!patchRes.ok) {
      throw new Error('Failed patching existing key row: ' + raw);
    }
  } else {
    ...
    if (!insertRes.ok) {
      throw new Error('Failed inserting new key row: ' + raw);
    }
  }
  ...
  if (!keyRow) {
    throw new Error('Key row missing after save');
  }

  if (keyRow.claimed !== true) {
    throw new Error('Key row did not save claimed=true');
  }

  if (keyRow.agent_slug !== slug) {
    throw new Error(
      `Key row did not save correct slug. Expected ${slug}, got ${keyRow.agent_slug}`
    );
  }
