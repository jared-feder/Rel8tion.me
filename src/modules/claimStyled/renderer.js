...
const STYLED_ASSETS = {
  introSign: 'https://rel8tion.me/wp-content/uploads/2026/04/sign-colorfulmockup.png'
};
  ...
  render(`
    <div>
      ...
      <div class="space-y-3">
        ...
        <button onclick="showForm()" class="w-full py-5 rounded-full bg-white/85 border border-slate-200 text-slate-700 font-black text-[17px] uppercase tracking-[-0.02em] shadow-sm active:scale-[0.99] transition-all">Set Up Manually</button>
      </div>
    </div>
  `);
}
...
export function showIntro() {
  render(`
    <div class="text-center">
      <div class="inline-flex items-center justify-center mb-6"><div class="px-4 py-2 rounded-full bg-white/60 border border-white/80 shadow-sm text-[11px] font-black uppercase tracking-[0.22em] text-slate-500">Agent Activation</div></div>
      ...
      <div class="rounded-[34px] border border-white/70 bg-white/55 shadow-[0_20px_60px_rgba(31,42,90,0.08)] p-5 md:p-6 mb-7 text-left backdrop-blur-sm">
        <div class="flex items-center gap-4">
          <div class="shrink-0">
            <div class="relative w-[140px] h-[125px] md:w-[145px] md:h-[145px] floating-chip">
              <div class="absolute inset-0 rounded-full blur-2xl opacity-40 bg-[radial-gradient(circle,#60a5fa_0%,#60a5fa_0%,transparent_75%)]"></div>
              <img src="${STYLED_ASSETS.introSign}" class="relative w-full h-full object-contain drop-shadow-[0_45px_45px_rgba(64,200,255,0.28)]" alt="Rel8tion Smart Sign">
            </div>
          </div>
          <div class="min-w-0">
            <div class="text-slate-900 text-[24px] md:text-[26px] leading-[1.0] font-black tracking-[-0.03em] mb-2">Our Smart Sign<br>Programs Itself</div>
            <p class="text-slate-500 text-[15px] md:text-[17px] leading-relaxed font-medium">No set up needed. Just tap it when you arrive at your open house and it does the rest. Automatically.</p>
          </div>
        </div>
      </div>
      <div class="rounded-[28px] border border-blue-100 bg-white/55 backdrop-blur-sm px-6 py-6 shadow-inner mb-7"><div class="text-slate-900 text-[19px] md:text-[20px] font-black tracking-[-0.02em]">Put it on your keychain and use it everyday.</div></div>
      <div class="space-y-3">
        ...
        <button onclick="skipToForm()" class="w-full py-5 rounded-full bg-white/85 border border-slate-200 text-slate-700 font-black text-[17px] md:text-[18px] uppercase tracking-[-0.02em] shadow-sm active:scale-[0.99] transition-all">🔵 No — Set Up Manually</button>
      </div>
    </div>
  `);
}
  ...
  render(`
    <div>
      ...
      <div class="space-y-3">
        ...
        <button onclick="showOtherListings()" class="w-full py-5 rounded-full bg-white/85 border border-slate-200 text-slate-700 font-black text-[17px] uppercase tracking-[-0.02em] shadow-sm active:scale-[0.99] transition-all">Show Other Listings Nearby</button>
  ...
  render(`
    <div>
      ...
      <div class="space-y-4 mb-8 max-h-[480px] overflow-y-auto pr-2">
        ${unique.map((h) => { const s = getStatus(h); return `
  ...
  render(`
    <div>
      ...
      <div class="space-y-3">
        ...
        <button onclick="showFullProfileForm()" class="w-full py-5 rounded-full bg-white/85 border border-slate-200 text-slate-700 font-black text-[17px] uppercase tracking-[-0.02em] shadow-sm active:scale-[0.99] transition-all">✍️ Complete Profile Now</button>
        <button onclick="showForm(state.detectedHouse?.brokerage || '')" class="w-full py-5 rounded-full bg-white/85 border border-slate-200 text-slate-700 font-black text-[17px] uppercase tracking-[-0.02em] shadow-sm active:scale-[0.99] transition-all">No — Let Me Edit My Info</button>
      </div>
    </div>
  `);
}
  ...
  render(`
    <div>
      ...
      <div class="space-y-3">
        ...
        <button onclick="showFullProfileForm('${esc(agent.brokerage || '')}', 'Update your full profile anytime below.')" class="w-full py-5 rounded-full bg-white/85 border border-slate-200 text-slate-700 font-black text-[17px] uppercase tracking-[-0.02em] shadow-sm transition-all">Complete / Update Full Profile</button>
      </div>
    </div>
  `);
}
  ...
  render(`
    <div>
      ...
      <div class="space-y-4 text-left">
        <input id="name" value="${esc(p.name || '')}" placeholder="Full Name *" class="w-full rounded-[20px] border border-slate-200 bg-white/80 px-5 py-4 text-[16px] font-semibold text-slate-900 outline-none focus:border-blue-400">
        <input id="phone" value="${esc(p.phone || '')}" placeholder="Phone Number *" class="w-full rounded-[20px] border border-slate-200 bg-white/80 px-5 py-4 text-[16px] font-semibold text-slate-900 outline-none focus:border-blue-400">
      </div>
      <div class="mt-6 space-y-3">
        ...
        <button onclick="startDetection()" class="w-full py-5 rounded-full bg-white/85 border border-slate-200 text-slate-700 font-black text-[17px] uppercase tracking-[-0.02em] shadow-sm active:scale-[0.99] transition-all">Try Detection Again</button>
      </div>
    </div>
  `);
}
  ...
  render(`
    <div>
      ...
      <div class="space-y-4 text-left">
        ...
        <input id="full_brokerage" value="${esc(brokerage)}" placeholder="Brokerage" class="w-full rounded-[20px] border border-slate-200 bg-white/80 px-5 py-4 text-[16px] font-semibold text-slate-900 outline-none focus:border-blue-400">
        ...
        <input id="full_instagram" value="${esc(instagram)}" placeholder="Instagram" class="w-full rounded-[20px] border border-slate-200 bg-white/80 px-5 py-4 text-[16px] font-semibold text-slate-900 outline-none focus:border-blue-400">
        <input id="full_website" value="${esc(website)}" placeholder="Website" class="w-full rounded-[20px] border border-slate-200 bg-white/80 px-5 py-4 text-[16px] font-semibold text-slate-900 outline-none focus:border-blue-400">
        <input id="full_facebook" value="${esc(facebook)}" placeholder="Facebook" class="w-full rounded-[20px] border border-slate-200 bg-white/80 px-5 py-4 text-[16px] font-semibold text-slate-900 outline-none focus:border-blue-400">
        <textarea id="full_bio" placeholder="Bio" class="w-full rounded-[20px] border border-slate-200 bg-white/80 px-5 py-4 text-[16px] font-semibold text-slate-900 outline-none focus:border-blue-400 min-h-[120px] resize-y">${esc(bio)}</textarea>
      </div>
      <div class="mt-6 space-y-3">
        ...
        <button onclick="autoActivate()" class="w-full py-5 rounded-full bg-white/85 border border-slate-200 text-slate-700 font-black text-[17px] uppercase tracking-[-0.02em] shadow-sm active:scale-[0.99] transition-all">Do This Later</button>
      </div>
    </div>
  `);
