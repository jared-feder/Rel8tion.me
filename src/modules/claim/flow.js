...
export async function confirmListing() {
  ...
  if (state.detectedHouse.agent || state.detectedHouse.agent_phone || state.detectedHouse.agent_email) {
    showVerifyAgent();
  } else {
    showForm(state.detectedHouse.brokerage || '');
  }
}
...
export async function autoActivate() {
  ...
  try {
    ...
    if (!existingAgent && phone_normalized) {
      existingAgent = await findAgentByPhoneNormalized(phone_normalized);
    }
    if (!existingAgent && h.agent_email) {
      existingAgent = await findAgentByEmail(h.agent_email);
    }
...
export async function init() {
  ...
  try {
    ...
    if (state.keyRecord?.claimed === true && state.keyRecord?.agent_slug) {
      if (state.prefilledAgent) {
        showAlreadyClaimed(state.prefilledAgent);
      } else {
        window.location.href = `${ROUTES.onboarding}?agent=${encodeURIComponent(state.keyRecord.agent_slug)}`;
      }
    } else {
      showIntro();
    }
