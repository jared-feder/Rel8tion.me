...
test('inventory and upcoming open-house payloads use trusted relationship contact data', () => {
  ...
  assert.equal(inventory.source_payload.has_upcoming_open_house, true);
  assert.equal(inventory.source_payload.open_house_tag_source, 'computed_earliest');
...
test('listing inventory replaces a stale tag with the next future collection event', () => {
  const nowIso = '2026-07-30T00:00:00.000Z';
  const record = oneKeyRecord({
    Computed: {
      OpenHousesEarliestStartTime: '2026-07-20T16:00:00Z',
      OpenHousesEarliestEndTime: '2026-07-20T18:00:00Z'
    },
    OpenHouses: [{
      StartTime: '2026-08-02T17:00:00Z',
      EndTime: '2026-08-02T19:00:00Z'
    }]
  });
  const window = openHouseWindow(record, nowIso);
  assert.equal(window.start, '2026-08-02T17:00:00Z');
  assert.equal(window.end, '2026-08-02T19:00:00Z');
  assert.equal(window.source, 'open_house_collection');

  const profile = relationshipProfile({ id:'agent-1', name:'Ruth Chalco', brokerage:'Example Realty' });
  const match = matchProfiles(record, [profile])[0];
  const inventory = inventoryPayload(record, match, nowIso);
  assert.equal(inventory.open_start, '2026-08-02T17:00:00Z');
  assert.equal(inventory.source_payload.has_upcoming_open_house, true);
});
