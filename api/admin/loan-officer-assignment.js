...
async function assignConfirmedVisit(visitId, loanOfficerUid) {
  ...
  const payload = {
    ...
    participant_email:profile.email || null,
    participant_company:profile.company_name || null,
    role:'loan_officer', responsibility:'financing_support', status:'assigned', is_primary:true,
...
async function handler(req, res) {
  try {
    ...
    if (action === 'assign_visit') {
      ...
      const houseRows = result.visit.open_house_id
        ? await supabaseRest(`open_houses?id=eq.${enc(result.visit.open_house_id)}&select=*&limit=1`).catch(() => [])
        : [];
      const scheduledEvent = {
        id:result.visit.open_house_event_id || result.visit.id,
        is_scheduled_visit:!result.visit.open_house_event_id,
        host_agent_slug:result.visit.agent_slug || '',
        start_time:result.visit.scheduled_start,
        end_time:result.visit.scheduled_end,
        setup_context:{ address:result.visit.property_address || '', agent_name:result.visit.agent_name || '', agent_phone:result.visit.agent_phone || '', agent_email:result.visit.agent_email || '' }
      };
      const notifications = body.notify === false ? { skipped:true } : await notifyAssignment({
        req, event:scheduledEvent, house:Array.isArray(houseRows) ? houseRows[0] || {} : {},
        loanOfficer:result.loan_officer,
        agent:{ name:result.visit.agent_name || '', phone:result.visit.agent_phone || '', email:result.visit.agent_email || '' }
      });
      sendJson(res, 200, { ok:true, action, ...result, notifications });
      return;
    }
