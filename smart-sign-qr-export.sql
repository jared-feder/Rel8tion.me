...
-- 5) OPTIONAL INSERT: create 1000 new Smart Sign inventory rows.
-- Change row_count below before running. Default sign batch: 1000.
-- This inserts into public.smart_sign_inventory only and does not touch existing rows.
with settings as (
  select 1000::integer as row_count
),
candidate_codes as (
  select distinct
    lower(substr(replace(gen_random_uuid()::text, '-', ''), 1, 12)) as public_code
  from settings
...
-- 6) OPTIONAL INSERT: create new Event Pass inventory rows.
...
-- 7) OPTIONAL UPDATE: fix Event Pass qr_url values only.
...
-- 8) OPTIONAL UPDATE: mark exported Event Pass rows as printed after printing is confirmed.
