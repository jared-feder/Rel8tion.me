...
- `[IMPLEMENTED]` Brokerage-specific outreach can override the outreach route provider with `SMS_TWILIO_OUTREACH_BROKERAGES`; Douglas Elliman is routed to Twilio/MMS while the rest of outreach can remain on Android.
- `[IMPLEMENTED]` REL8TION COMMAND shows the outreach approval gate and provides Approve send/Hold send controls for generated/rendered due rows before automation can send them.
