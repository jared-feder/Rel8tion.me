...
| Event Pass closeout can start kit plus required service checkout. | `[PARTIAL]` | `/open-house-kit` explains the Open House Kit, requires either monthly or paid-for-the-year service when checking out for the kit, and keeps loan officer contact as the no-purchase path. The checkout API uses Stripe Checkout in subscription mode, defaults the one-time kit line item to price `price_1TYtd12LIj1DZULXtTeeeYSm`, and adds either `STRIPE_EVENT_PASS_MONTHLY_PRICE_ID` or `STRIPE_EVENT_PASS_ANNUAL_PRICE_ID` as the required recurring service line item. Live Stripe secret/monthly/annual env vars and product pricing still need verification. |
