...
- `[IMPLEMENTED]` Assignment SMS includes the registered campaign's opt-out disclosure for both agent and loan-officer recipients. Event transactional routing and central suppression remain in effect. Provider acceptance is distinct from delivery; carrier filtering may still require Twilio review.
...
- `[IMPLEMENTED]` A normal Event Pass remains Event Pass hardware. Active-event operation remains available for the authorized host, while permanent dashboard, check-in/disclosure history, analytics/reporting, digital-profile/card, and assisted-follow-up access requires an active REL8TION Agent membership. The reusable Smart Sign and custom company-branded Rel8tionChips are separate one-time Open House Kit hardware.
...
- `[IMPLEMENTED]` `/agent-home` is the private owner dashboard for normal claimed agent NFC scans and paid, idle Event Pass membership access.
...
- `[IMPLEMENTED]` `/event-pass-reuse` is the full-screen continuation route when a previously used normal Event Pass has no active REL8TION Agent entitlement. It preserves the physical pass and selected-listing context and offers loan-officer-sponsored limited use first when the server can resolve an active sponsor; the agent must consent for that event before the server creates sponsor visibility. The sponsor is the active loan officer assigned to the pass's earliest event, falling back to the canonical Brian Puls profile when that initial event has no usable assignment. Sponsored access is free and event-only: the live dashboard works for that open house, the closeout recap is server-emailed to the agent, and the required compliance/audit rows remain preserved internally, but the event does not remain in the unpaid agent's permanent dashboard, disclosure library, analytics, or reports. Closeout loads the authoritative check-ins and must successfully deliver and record that recap before it ends the event; failures leave the event active for correction and retry. The full REL8TION choice uses the catalog-controlled `$29/month` Agent plan for permanent history and returns successful or canceled Checkout to the interrupted registration instead of asking for another NFC tap.
- `[IMPLEMENTED]` The paid Event Pass return does not create a new agent. Checkout and activation remain bound to the pass's verified `agent_slug`; after the preserved event activates, the success page offers the live dashboard, profile completion, and the public digital-card preview. Profile completion edits the same canonical agent and safely returns to the live dashboard so event context is not lost.
...
- `[IMPLEMENTED]` NFC lifecycle is stateful: a live pass opens its event dashboard; an ended or otherwise idle pass opens the locked agent's Event Pass membership home. That home presents the public digital profile plus a Start Open House action bound to the same Event Pass QR/NFC pair. Without an active REL8TION Agent entitlement, it shows membership checkout and withholds private event history. It does not represent the Event Pass as custom-branded Rel8tionChip hardware.
- `[IMPLEMENTED]` Ended-event commercial actions follow the same entitlement boundary: only a verified active member sees the `$199` one-time Open House Kit hardware offer. Sponsored event-only and unpaid normal Event Pass users see the `$29/month` REL8TION Agent activation path instead.
...
- `[IMPLEMENTED]` An active `$29/month` REL8TION Agent member may buy the physical Open House Kit for the catalog-controlled `$199` one-time price without creating another subscription. `/api/checkout/open-house-kit` exposes this private member-hardware offer only after validating the claimed NFC session and active membership server-side; Checkout contains the existing `open_house_kit` price only and omits subscription data. The kit contains the reusable Smart Sign and custom company-branded Rel8tionChips.
...
An active Event Pass NFC opens only its currently hosted live-event dashboard. When idle, the same claimed paid-agent Event Pass may open `/agent-home` for membership and owner controls. Both paths require the short-lived server-issued NFC session and current key ownership; copied URLs and browser-local host declarations are not authorization. Event Pass NFC does not enroll the SMS-backed normal-keychain phone session, and reuse/private history still requires the paid agent entitlement described above. The physical Event Pass remains distinct from separately purchased company-branded Rel8tionChip/Open House Kit hardware.
