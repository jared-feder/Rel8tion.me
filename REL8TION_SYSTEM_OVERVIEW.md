...
The root `/admin` compatibility page redirects directly to the canonical REL8TION COMMAND document at `/apps/rel8tion-app/admin.html`; it does not inject fetched HTML into the current document.
...
- `[IMPLEMENTED]` Buyer property media in `/event` opens the dedicated REL8TION `/l/:open_house_id` property experience. It provides an expandable multi-photo gallery, rich property facts and description, features, host context, verified MLS access, and a return link to the same event check-in.
- `[IMPLEMENTED]` Rich buyer property content is modeled as a durable, server-managed `open_house_property_profiles` row keyed to the source `open_houses.id`. The route refreshes stale OneKey-backed profiles and keeps the saved gallery/data usable between upstream requests; buyers never receive the private raw upstream payload or direct table access.
...
- `[IMPLEMENTED]` REL8TION COMMAND Agent Performance is an additive unified Agent Board spanning claimed identities, production rankings, outreach and conversations, leads, listing inventory, listing-feed contacts, and upcoming open houses. It keeps agents visible even when they have neither messages nor current listings, preserves expandable source detail on each card, and links to the unchanged source workspaces instead of replacing their features.
- `[IMPLEMENTED]` REL8TION COMMAND Open Houses exposes a relationship-only upcoming-open-house selector for agents with existing conversation threads. Admins can choose one event per agent, edit the suggested Event Pass copy, confirm the batch, and send through the existing protected manual-reply workflow rather than bypassing suppression, opt-out, provider, delivery-log, reply-threading, or quiet-hour controls.
- `[IMPLEMENTED]` REL8TION COMMAND agent photos can be enlarged from admin Agent Performance, outreach, and confirmed-open-house views. This lightbox is an admin interaction and is intentionally separate from the buyer experience.
...
- `[IMPLEMENTED]` Rel8tionOS outreach detail combines sent initial/follow-up queue copy with stored reply rows into one chronological conversation. Queue-only contacts remain visible in the all-threads view, and each thread carries normalized agent identity and photo context for the separate Rel8tionOS frontend.
...
- `[IMPLEMENTED]` REL8TION COMMAND's Signs area includes an admin-only Event Pass Freshener that scans the printed QR by camera, image, or pasted code. One scan ends the old live event/coverage, clears old host and Event Pass NFC ownership, removes same-sign Event Pass QR aliases, and cancels pending activation sessions while preserving event/check-in/disclosure history, the printed code, sponsor configuration, and reuse settings.
- `[IMPLEMENTED]` An unlinked Event Pass inventory row never uses the legacy `smart_signs.public_code` fallback. This prevents a freshly reset pass from silently relinking to a stale prior-host sign; non-Event-Pass Smart Sign legacy fallback is unchanged.
...
- `[INTENDED]` A formal invite/request/accept workflow and remote availability queue are not built.
- `[IMPLEMENTED]` Persistent agent-to-loan-officer links live in `agent_loan_officer_relationships`. Live-event, scheduled-visit, confirmed-outreach, and automatic coverage assignment may select only an active linked loan officer for the hosting agent; both REL8TION COMMAND option lists and the server assignment boundary enforce the same rule.
...
- `[IMPLEMENTED]` The host-agent live event dashboard uses those protected REL8TION COMMAND Open Houses controls for Add or Replace Loan Officer and deep-links to the current event. It does not arm the retired loan-officer Rel8tionChip handoff, and it clears stale pending handoff state when the dashboard opens.
...
- `[PARTIAL]` REL8TION COMMAND at `/admin` is the operational admin dashboard. It includes a Buyer Finder workspace backed by `/api/admin/buyer-home-finder` for admin-only searching, filtering, sorting, and printable buyer-report generation across upcoming open-house/listing records, plus the admin-only Event Pass Freshener scanner in Signs.
- `[IMPLEMENTED]` REL8TION COMMAND has a dedicated Open Houses workspace. Live events render before scheduled coverage and recent history, link directly to the event dashboard, and no longer sit below sign inventory. An admin can cancel a scheduled/confirmed open house with explicit confirmation; the service-role event action preserves history, releases field participants and availability blocks, restores accepted outreach to interested status, and ends any matching live event through the shared sign and loan-officer cleanup path.
- `[IMPLEMENTED]` The Open Houses workspace also renders upcoming Agent Board events before history, merges outreach/listing-inventory/open-house-feed context, sorts future events soonest first, and links each agent back to the same unified identity and conversation.
...
- `[IMPLEMENTED]` Agent Ranking distinguishes imported ListReports active-listing activity from actual current listing records in REL8TION. Protected `agent_listing_inventory` holds active/pending OneKey listings for ranked agents, claimed agents, and agents with positive outreach relationship statuses. The two-hour production cron matches by trusted contact or exact agent-name/brokerage identity and retains stale rows as inactive audit history. Ranked-only inventory is visible in the admin profile, while listing marketing eligibility is limited server-side to claimed/worked-with or positive-interest relationships. The code defaults to inventory-only shadow mode, but production enabled `AGENT_LISTING_INVENTORY_PROMOTE_OPEN_HOUSES` on 2026-08-02 so matched upcoming open houses are now also upserted into `open_houses`; the first promoted run wrote 77 rows and confirmed coverage for all currently upcoming inventory, including 51 listings tied to `worked_with` agents. The explicit manual/non-sending outreach action remains separately gated and was not changed by this activation. No automatic sending is introduced.
...
- `[NEEDS VERIFICATION]` `agent_listing_inventory` migration source exists with RLS enabled, no anon/authenticated access, explicit service-role access, and indexes for current agent/listing/open-house lookup. Application of the migration and the first relationship listing scan are not proven live.
...
- `[IMPLEMENTED]` As of 2026-07-22, the Vercel send cron and `send-agent-outreach` Edge Function hard-cap automatic sends at 5 per run, 20 per rolling hour, and 150 per rolling 24 hours. The seven-day opt-out health gate activates at 1% after at least 20 logged outreach sends. The current operator state is Live/manual with zero unsent future automatic rows; 82 future rows are manual, including 57 ready and 25 awaiting mockups.
...
- `[IMPLEMENTED]` Android inbound SMS is shown in REL8TION COMMAND only when its sender resolves to an outreach queue row with an agent identity. Unmatched Android messages remain stored for diagnostics and suppression handling but are excluded from the operational inbox, counts, latest-inbound cards, and thread detail.
...
The loan-officer dashboard overview and full Open Houses + Buyers section both render listing photos and street addresses from the assigned outreach queue context. Missing visit contact fields hydrate from that same linked outreach context so overview cards show the hosting agent's name, company/brokerage, phone, and one-tap Call, Text, and Directions actions. The dashboard header renders the active verified profile's public headshot.
...
The agent event dashboard is not a loan-officer workspace. It can text the assigned loan officer to request guidance and can display a limited reviewed outcome, but it cannot open the LO guidance-entry page, create property-fit scenarios, or receive private cap/rate/term/down-payment/rent assumptions. `/lo-affordability-guidance` and all guidance/scenario write actions require a Supabase access token that resolves server-side to the requested active loan-officer profile.
