...
| Event Pass closeout can start kit or monthly checkout. | `[PARTIAL]` | `/open-house-kit` explains the Open House Kit, offers one-time kit purchase, monthly Event Pass/dashboard service, or loan officer contact, and posts to `/api/checkout/open-house-kit`. The checkout API uses Stripe Checkout, defaults the one-time kit to price `price_1TYtd12LIj1DZULXtTeeeYSm`, and supports monthly checkout through `STRIPE_EVENT_PASS_MONTHLY_PRICE_ID` or `STRIPE_EVENT_PASS_MONTHLY_PAYMENT_LINK`. Live Stripe secret/monthly env vars and product pricing still need verification. |
