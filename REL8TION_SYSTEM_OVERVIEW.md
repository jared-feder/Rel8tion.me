...
- `[PARTIAL]` `/loan-officer-dashboard` is the permanent loan-officer owner dashboard and directly supports adding/removing dated availability windows with service ZIP and radius. Real access uses Supabase passwordless email authentication and requires the session email to match the approved verified profile. Its My Agents section derives established relationships from covered visits and provides one automatic combined Event Pass plus remote live-financing support offer; agent sending and LO chat replies enforce the same ownership check server-side. `/lo-field-dashboard` and `/field-dashboard?role=loan_officer` remain the detailed coverage, messaging, and field-operations workspace. Durable delivery logs, recurring weekly templates, and ownership enforcement on older availability/field APIs remain incomplete.
