...
- `[PARTIAL]` `/loan-officer-dashboard` is the permanent loan-officer owner dashboard and directly supports adding/removing dated availability windows with service ZIP and radius. Its My Agents section derives established relationships from covered visits and provides one automatic combined Event Pass plus remote live-financing support offer. `/lo-field-dashboard` and `/field-dashboard?role=loan_officer` remain the detailed coverage, messaging, and field-operations workspace. Durable delivery logs, recurring weekly templates, and authenticated ownership remain incomplete; automatic agent sending must not be promoted publicly before ownership enforcement.
