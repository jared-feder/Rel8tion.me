...
- Tailwind runtime on all active app HTML pages, with preflight disabled where existing custom static CSS needs to remain stable
...
- `api/admin/event-action.js`
- `api/admin/loan-officer-assignment.js`
...
Role: public smart sign and Event Pass resolver. `/s` and `/sign` preserve the smart sign behavior; `/pass` uses the same resolver module in Event Pass mode so printed Event Pass QR batches can use `https://app.rel8tion.me/pass?code=PUBLIC_CODE`.

`[IMPLEMENTED]` Confirmed repo behavior:

- Reads `code` from URL.
- Resolves `smart_sign_inventory.public_code` first. For existing `/s` and `/sign` smart-sign routes only, the resolver still allows legacy `smart_signs.public_code` fallback so old live smart sign links keep working.
...
- Sends buyer and agent SMS through `send-lead-sms` only after local check-in validation passes. Future buyer confirmation SMS uses an outcome-based `/l/<open_house_id-or-event_id>` link instead of source-specific Citysnap/OneKey rules: the app asks `api/open-house-link.js` to verify the saved external destination, records the outcome in check-in metadata, labels the SMS link as either an MLS page or REL8TION property page, and only shows the external MLS button on the landing page when the destination verifies successfully.
...
- If the linked open house has a saved listing URL, the buyer confirmation SMS includes the short REL8TION listing link.
...
- The LO affordability page and agent Property Fit Checker both capture annual property taxes and annual homeowners insurance on property scenarios because those property expenses materially change the monthly payment. Money fields use flexible dollar input instead of fixed browser steps, and the LO guidance save path returns to the originating dashboard with `return_to` when provided.
...
- Event Pass rows must use `/pass?code=...` URLs, and `/pass` resolves them from `smart_sign_inventory.public_code` before any sign behavior.
...
- The automatic sender no longer applies a cross-listing same-phone duplicate cooldown. As of 2026-05-29, separate listings/open houses can send separate initial outreach to the same agent phone, with separate listing photos/mockups. Opt-out/provider suppression, Android opt-out status, invalid phone handling, quiet hours, image gates, and expired/started open-house guards still apply. Historical `blocked_duplicate` rows remain historical until explicitly requeued.
...
- Excludes rows that appear opted out, manually sent, manually skipped, blocked for invalid/opt-out/image reasons, expired, already handled by the existing status fields, missing an actual listing image, or missing a rendered Rel8tion outreach photo. `blocked_duplicate` is no longer treated as a terminal handled state because same-agent separate-listing outreach is allowed.
...
- `send-agent-outreach` sends outbound SMS with quiet hours, invalid phone handling, opt-out handling, follow-up status, expiration rules, image gates, and admin-scheduled drip follow-ups when `review_status = drip_scheduled`. The 2026-05-20 version fetches due pending initial/follow-up rows directly instead of scanning oldest generated/rendered history first, and supports `dry_run` / `diagnostic_no_send` output for no-SMS verification. The 2026-05-21 version passes Twilio `StatusCallback` URLs for initial and follow-up messages and initializes delivery status fields from Twilio's immediate response. The 2026-05-29 version removes the same-phone duplicate cooldown so one agent can receive separate outreach for separate listings/open houses.
...
- `[PARTIAL]` `apps/rel8tion-app/admin.html` is the admin-keychain/token-protected REL8TION COMMAND dashboard. It has a command-header layout, area selector, live stats, hot-list-style outreach cards with agent/listing imagery, inline reply composers, interested/yes triage, true-open-house confirmation, accepted-open-house controls, drip scheduling, thread history, Leads, Agent CRM, smart signs/events, smart-sign detach-to-fresh controls, loan officer assignment/profile/session views, accepted/confirmed field visits, payments-needed setup, and outreach reports. The Outreach view prioritizes reply conversations first and moves the raw queue snapshot to the bottom as a reference section instead of leading with the large queued-record count. `/api/admin/outreach-search` searches the full outreach queue, token-matches address phrases, handles admin-header browser preflight, and merges listing-only `listing_agents` rows for matched open houses so co-listing agents can be found even when only one has an outreach queue row. The route treats queue, open-house, and listing-agent lookups as independent data sources and returns partial rows with warnings instead of failing the whole search if one source has a temporary issue; `scripts/verify-command-search.cjs` covers preflight/unauthenticated behavior and optional authenticated production assertions for known property searches.
...
- `[IMPLEMENTED]` `/api/admin/outreach-search` provides protected server-side outreach/listing search for REL8TION COMMAND. The admin Outreach search box uses this endpoint so older queue rows and listing-only co-agents can be found by property address, agent, phone, email, open-house id, outreach code, or report note even when they are outside the currently loaded inbox/dashboard slice.
- `[IMPLEMENTED]` `/api/admin/auth` validates `ADMIN_KEYCHAIN_UIDS` or fallback admin token access for the dashboard.
- `[PARTIAL]` The admin page has a local Dashboard Lock concept after `/api/admin/auth` succeeds. It stores a browser-local 4-digit PIN hash and can optionally register a platform passkey/phone unlock with WebAuthn for Face ID, fingerprint, or device-passcode prompts. This is not final security because the privileged admin APIs still authorize with the existing admin UID/token headers; a hardened rollout should add a server-issued, short-lived unlock session and require it in `lib/admin-auth.js`.
...
- `[PARTIAL]` Broader write controls for sign inventory beyond detach-to-fresh, CRM updates, external calendar conflict checking, billing automation, and deep analytics drilldowns remain future work.
...
- `[PARTIAL]` In-app event conversation logging exists through `event_conversations`, `event_conversation_messages`, `/api/event-chat/*`, `/event-chat`, and `/field-dashboard`. Loan-officer dashboard messages create a buyer access token in conversation metadata and SMS the buyer an `/event-chat?cid=...&token=...` return link; buyer replies from that page can notify the LO by SMS. Realtime delivery, push alerts, video, and hardened auth are not built.
...
- `[IMPLEMENTED]` `smart-sign-qr-export.sql` now uses `public.smart_sign_inventory` only. Event Pass rows print `/pass?code=...`; Smart Sign rows may retain existing `/s.html` or `/s` URLs. The file includes optional Smart Sign and Event Pass print-batch inserts that create only new `smart_sign_inventory` rows, plus Event Pass qr_url-fix and mark-printed blocks.
...
| Event Pass closeout can start kit plus required service checkout. | `[PARTIAL]` | `/open-house-kit` explains the Open House Kit, requires either monthly or paid-for-the-year service when checking out for the kit, and keeps loan officer contact as the no-purchase path. The page states the kit is expected within 14 days or may be delivered sooner by Moe personally when scheduling allows, and frames the offer as Version 1 pricing that locks in the selected rate while future upgrades and price increases are expected. The checkout API uses Stripe Checkout in subscription mode, defaults the one-time kit line item to price `price_1TYtd12LIj1DZULXtTeeeYSm`, defaults monthly service to `price_1TYtd52LIj1DZULX8ipqLz9X`, defaults annual service to `price_1TYtd62LIj1DZULXy58hBZ0I`, collects US shipping address/phone in Stripe Checkout, applies REL8TION checkout branding, and adds fulfillment/rate-lock custom text to the hosted Stripe page. Live Stripe secret/env deployment and product pricing still need verification. |
...
| Buyer, agent, and loan officer should be able to communicate in a richer live modal. | `[PARTIAL]` | In-app event conversation logging exists through `event_conversations`, `event_conversation_messages`, `/api/event-chat/*`, `/event-chat`, and `/field-dashboard`; LO-to-buyer SMS return links are implemented, while realtime delivery, push alerts, video, and hardened auth are not built. |
...
| Full admin action dashboard is desired but not fully built. | `[PARTIAL]` | REL8TION COMMAND exists at `/admin` with leads, outreach replies, interested/confirmed/accepted open house workflow, confirmed open house reports/PDF-style export, drip scheduling, sign event closeout, smart-sign detach-to-fresh, live LO assignment/removal, LO profile soft-removal, and live read/reporting cards, but broader sign inventory record edits, CRM edits, LO calendar/availability changes, and billing automation remain unfinished. |
...
| QR source is unified for printing. | `[IMPLEMENTED]` | Printable QR exports use `public.smart_sign_inventory.public_code` only. `inventory_type` allows only `smart_sign` and `event_pass`; event passes print and resolve through `/pass`, and `smart_signs.public_code` is not used for new QR generation. |
