...
- `[PARTIAL]` Agent Ranking distinguishes imported ListReports active-listing activity from actual current listing records in REL8TION. A protected `agent_listing_inventory` is intended to hold active/pending OneKey listings for all ranked agents plus claimed agents and agents with positive outreach relationship statuses. The two-hour server cron matches by trusted contact or exact agent-name/brokerage identity and retains stale rows as inactive audit history. Ranked-only inventory is visible in the admin profile, while listing marketing eligibility is limited server-side to claimed/worked-with or positive-interest relationships. The cron is disabled by default and, when enabled, operates in inventory-only shadow mode; promotion into `open_houses` and the explicit manual outreach action each require separate production flags. No automatic sending is introduced. This lets REL8TION validate listings before enrichment without changing existing open-house/outreach behavior. Production schema/cron/live data remain `[NEEDS VERIFICATION]`.
