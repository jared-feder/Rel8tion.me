...
- `[IMPLEMENTED]` Outreach recovery safety includes a 30-day same-phone cooldown, rolling opt-out health gate, 7-day maximum age for missed-open-house outreach, shorter permission-oriented first contact, follow-ups disabled, and initial MMS disabled by default. Android Gateway is text-only; automated outbound MMS requires verified Twilio capability.
...
- `[PARTIAL]` As of 2026-07-14, Twilio route separation is configured: operational SMS uses `+15168885461`, and outreach uses `+18448211802` through Messaging Service `MG8d7ec49cf1d6d231080b7f870a10eb0b`. An owner-only one-image MMS was accepted with sender `+18448211802` and SID `MM395033030537bb1de5b4b0b6489d7cdd`; inbound reply handling and toll-free verification/use-case approval still require verification, so automatic outreach remains globally paused.
