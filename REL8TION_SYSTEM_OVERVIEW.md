...
- `[IMPLEMENTED]` `/api/event-pass/action` is the server authorization boundary for normal Event Pass registration. It derives the host from the claimed keychain, compares stored agent identity with the listing agent, permits an explicitly confirmed substitute only when the stored brokerage matches, then locks the event, sign, and inventory to that agent. Sponsored activation uses the same authorization module; free-form brokerage text alone cannot authorize a substitute.
...
- `[PARTIAL]` Test branch `codex/disclosure-immutable-audit` proposes v3 signing evidence: an audit receipt printed on the PDF, exact versioned consent text, a write-once hash-bound Storage key, and a database-computed event hash in `disclosure_signing_events`. The ledger is forced-RLS, service-role-only, append-only even to its owner, and canonical per check-in/packet version.
- `[PARTIAL]` Server request evidence is attached only to a recent check-in that contains explicit e-sign consent. Older v1/v2 packets may be converted without overwriting history, but the new audit record is labeled `legacy_import` with no signer IP, server user agent, request ID, or reconstructed consent text. This avoids presenting a later downloader as the original signer.
- `[NEEDS VERIFICATION]` The v3 migration and route are not production-live. Production continues to use the verified v2 behavior until the branch is reviewed, the isolated migration is applied, and a real consented signing is verified end to end. IP/user-agent retention and final legal sufficiency require a written business/compliance policy.
