...
- `[IMPLEMENTED]` Automated outreach sends are throttled for provider safety: the Vercel send cron defaults to 3 per run, and the `send-agent-outreach` Edge Function hard-caps automatic sends with default limits of 3 per run and 6 per rolling hour. Automatic initial and follow-up sends do not require `approved_for_send=true`; eligible rows are `send_mode=automatic`, generated, rendered, due, with a listing photo and pending SMS copy.
...
- `[IMPLEMENTED]` Production outreach is intended to run through Twilio with `SMS_OUTREACH_PROVIDER=twilio`, `SMS_EVENTS_PROVIDER=twilio`, and `SMS_PROVIDER=twilio`; brokerage-specific `SMS_TWILIO_OUTREACH_BROKERAGES` remains available only as an override if the default outreach provider changes later.
- `[IMPLEMENTED]` REL8TION COMMAND shows Cron ready rows and provides Pause cron/Resume cron controls that intentionally change `send_mode`.
...
- `[IMPLEMENTED]` As of 2026-06-24, Twilio SMS is restored via `SMS_PROVIDER=twilio`, `SMS_OUTREACH_PROVIDER=twilio`, `SMS_EVENTS_PROVIDER=twilio`, and `TWILIO_PHONE=+15168885461`. Use the Android Gateway only as a temporary outreach fallback if Twilio provider health requires it.
