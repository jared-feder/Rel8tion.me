...
- `[IMPLEMENTED]` The Reports section builds searchable upcoming/previous confirmed open house cards and CSV exports from `agent_outreach_queue`, `agent_outreach_replies`, `field_demo_visits`, `field_demo_visit_participants`, `event_loan_officer_sessions`, and `open_houses` when available. Rows include confirmed date, open house times, property details, listing/mockup/agent photo URLs, conversation snapshot, and LO coverage when recorded.
