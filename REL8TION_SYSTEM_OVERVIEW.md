...
- `[PARTIAL]` `/loan-officer-dashboard` is the permanent loan-officer owner dashboard and directly supports adding/removing dated availability windows with service ZIP and radius. It also derives worked agents from covered visits and creates LO-owned Event Pass, remote-coverage, or in-person-coverage outreach for manual SMS/email delivery. `/lo-field-dashboard` and `/field-dashboard?role=loan_officer` remain the detailed coverage, messaging, and field-operations workspace. Saved outreach records, recurring weekly templates, and authenticated ownership remain incomplete.
