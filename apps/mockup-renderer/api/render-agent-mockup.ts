...
function buildLink(agentSlug: string | null): string {
  return agentSlug
    ? `${env.publicBaseUrl}/a?agent=${encodeURIComponent(agentSlug)}`
    : `${env.publicBaseUrl}/`;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .select("id,agent_name,agent_photo_url,brokerage,address,city,state,zip,open_start,open_end,property_image_url,agent_slug")
  ...
  for (const row of (data || []) as QueueRow[]) {
    try {
      const jpg = await renderMockupJpg({
        ...
        rel8tionUrl: buildLink(row.agent_slug)
      });
