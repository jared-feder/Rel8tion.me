...
- `[IMPLEMENTED]` `/k` now treats an active `verified_profiles.uid` scan as an LO operations keychain and opens `/lo-field-dashboard?uid=...` directly instead of falling through to buyer/profile routing. Pending event LO sign-in from the agent dashboard still takes priority and creates/updates `event_loan_officer_sessions`.
...
| Loan officer tag scan verifies event support and opens LO dashboard. | `[IMPLEMENTED]` | Agent dashboard arms pending LO session; `/k` verifies `verified_profiles` and writes `event_loan_officer_sessions`. Standalone active LO scans route to `/lo-field-dashboard?uid=...`. |
