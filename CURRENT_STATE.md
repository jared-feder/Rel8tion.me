...
- `[IMPLEMENTED]` Production outreach is intended to run through Twilio on the scheduled cron lane with `SMS_OUTREACH_PROVIDER=twilio`, `SMS_EVENTS_PROVIDER=twilio`, and `SMS_PROVIDER=twilio`; `SMS_TWILIO_OUTREACH_BROKERAGES` remains available only as an optional brokerage override if the default outreach provider changes later.
- `[IMPLEMENTED]` Root cron code includes outreach generation and send endpoints. Outreach sending is throttled for provider safety: the send cron defaults to 3 per run, and the `send-agent-outreach` Edge Function hard-caps automatic sends with `OUTREACH_SEND_MAX_PER_RUN` defaulting to 3 and `OUTREACH_SEND_MAX_PER_HOUR` defaulting to 6. Automatic initial and follow-up sends do not require `approved_for_send=true`; eligible rows are `send_mode=automatic`, generated, rendered, due, with a listing photo and pending SMS copy.
- `[IMPLEMENTED]` REL8TION COMMAND surfaces generated/rendered due outreach rows as Cron ready and can explicitly Pause cron/Resume cron by changing `send_mode`. Do not reintroduce a hidden approval gate for normal cron sends without owner confirmation.
...
- `[INTENDED]` If Twilio provider health requires a temporary fallback, switch outreach with `SMS_OUTREACH_PROVIDER=android_gateway` while keeping event/owner/system texts on `SMS_EVENTS_PROVIDER=twilio`.
