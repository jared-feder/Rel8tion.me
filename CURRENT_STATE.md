...
- `[IMPLEMENTED]` `rel8tion_runtime_settings.key='outreach_release_window'` can temporarily authorize cron-driven health-gate bypass for a bounded `through_open_start` and `expires_at`. The sender applies the date ceiling to its query, so a release cannot spill into later open-house dates; all recipient suppression and volume ceilings remain enforced.
- `[IMPLEMENTED]` On 2026-08-05, the owner directed every eligible August 8 row to go out the same day. The date-scoped release completed at 4:53 PM ET with zero pending and was disabled for overnight safety. Final August 8 queue statuses were 66 sent, 16 duplicate blocks, 3 manual-SMS skips, and 1 expired row. For the 59 August 8 messages sent during the scoped release, final checkpoint callbacks were 54 delivered, 3 sent, and 2 failed with Twilio 21614 because the destinations were not valid mobile numbers; neither failure was retried. August 9 and later were excluded throughout the August 5 release.
- `[IMPLEMENTED]` The owner subsequently added August 9 rows with sending to begin on 2026-08-06, not August 5. The active heartbeat remains August-8-only today, stays inert overnight, and at 8:00 AM ET on August 6 expands `through_open_start` to `2026-08-10T04:00:00Z` with expiration at 9:00 PM ET that day. Thirty-one eligible August 9 rows were pending when scheduled; August 10 and later remain excluded.
