...
- `[VERIFIED]` Local syntax checks, route-map verification, pricing verification, Event Pass authorization/reuse tests, the new nine-case Rel8tionChip membership suite, and the mobile viewport suite pass. Production deployment, real Stripe checkout, signed webhook delivery, and a physical Event Pass tap remain `[NEEDS VERIFICATION]`.
