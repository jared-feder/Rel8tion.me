...
## 2026-08-01: Admin agent-photo viewer and buyer property experience

- `[IMPLEMENTED]` Agent photos in REL8TION COMMAND's Agent Performance, outreach, and confirmed-open-house views are admin-only enlargement controls. Selecting a photo opens a keyboard-accessible lightbox with the agent identity and brokerage; this behavior is not added to the buyer check-in page.
- `[IMPLEMENTED]` Buyer-facing property media on `/event` links to the existing REL8TION property route at `/l/:open_house_id`, preserving the live event id so the buyer can return to check-in. After check-in, the Property Snapshot includes a dedicated **Explore This Property** action.
- `[IMPLEMENTED]` The REL8TION property page now renders a full responsive gallery, expandable photos with previous/next keyboard controls, property facts, stored description, features, host/brokerage contact context, MLS access when verified, and a return-to-check-in action.
- `[IMPLEMENTED]` `open_house_property_profiles` is the server-managed durable property record keyed to `open_houses.id`. It stores the multi-photo gallery, description, property type/status, year built, lot size, taxes, HOA, price per square foot, features, listing/host context, and a private upstream snapshot. The buyer route reads a sanitized profile through the server API; anon/authenticated roles have no direct table access.
- `[IMPLEMENTED]` A missing or stale property profile is lazily enriched from the current OneKey listing record and image collection, then upserted for reuse. If enrichment or the profile table is unavailable, `/l/:open_house_id` gracefully falls back to the existing `open_houses` row and its saved image.
- `[NEEDS VERIFICATION]` Migration `20260801055717_open_house_property_profiles.sql` must be applied and the route deployed before durable production storage is live. Checked-in code alone does not prove production schema or deployment state.
- `[NEEDS VERIFICATION]` These source changes are not proven deployed. After deployment, verify an authenticated admin agent-photo enlargement and one designated live Event Pass buyer flow on mobile, including multiple property photos, gallery navigation, and return to the same check-in event.

## 2026-08-01: Relationship open-house messaging

- `[IMPLEMENTED]` REL8TION COMMAND's Open Houses workspace separates upcoming open houses for agents with an existing REL8TION conversation from other listing-feed/ranking matches. The relationship list is sorted soonest first and keeps direct Conversation and Listing access.
- `[IMPLEMENTED]` Admins can select one upcoming open house per agent, review or edit the personalized Event Pass SMS copy, and send the selected messages only after a batch confirmation. Sends reuse the existing protected manual-reply route, so global opt-out/suppression checks, provider routing, delivery logging, reply threading, and quiet-hour enforcement remain centralized.
- `[IMPLEMENTED]` Known opt-outs, missing conversation links, and invalid phone numbers are disabled in the selector. Selecting another open house for the same agent replaces the earlier selection to prevent multiple messages to one agent in the same batch.
- `[NEEDS VERIFICATION]` The source change is not proven deployed. Before production use, deploy the current REL8TION COMMAND build and send a single designated test message outside quiet hours; do not use the batch action as an automatic-outreach substitute.

## 2026-07-31: Rel8tionOS agent-context data foundation

- `[IMPLEMENTED]` The private Rel8tionOS outreach API reconstructs each chronological conversation from the queue's sent initial/follow-up messages plus stored reply rows. Queue-only outreach contacts are included in the all-threads view, and serialized agents now include normalized identity and photo context.
- `[IMPLEMENTED]` The relationship listing scraper reads both OneKey's computed earliest event and open-house collections, ignores stale windows, stores the soonest active/future window, and tags inventory payloads with `has_upcoming_open_house` plus the tag source.
- `[IMPLEMENTED]` REL8TION COMMAND now provides Agent Performance as a unified Agent Board. It combines claimed agents, rankings, outreach, conversation summaries, leads, protected listing inventory, listing-feed agents, and upcoming open houses instead of hiding agents that lack one particular source.
- `[IMPLEMENTED]` Agent names throughout outreach and confirmed/open-house views link into Agent Board. Each Agent Board record preserves full expandable outreach-thread, upcoming-open-house, and known-listing collections alongside outreach/message/listing/keychain/lead metrics, Conversation access, and a full ranking profile when available.
- `[IMPLEMENTED]` Agent Board is additive. The original Outreach, Open Houses, Agent Rankings, agent website editor, and operational actions remain available and are linked from the centralized board; no existing feature is intentionally replaced by a summary-only view.
- `[IMPLEMENTED]` REL8TION COMMAND Open Houses includes upcoming events from Agent Board before scheduled coverage and event history, sorted soonest first.
- `[NEEDS VERIFICATION]` These frontend/API changes are not proven deployed. No database migration or production-data write was run for this Agent Board work. The separately prepared listing-inventory cron target and migration remain untracked, production listing-inventory data is unverified, and the sync remains disabled by default.

## 2026-07-29: Code-free live-event loan-officer management

- `[IMPLEMENTED]` The host-agent live event dashboard no longer starts the retired loan-officer Rel8tionChip sign-in flow when Add or Replace Loan Officer is selected.
- `[IMPLEMENTED]` Live-event loan-officer changes now route directly to the protected REL8TION COMMAND Open Houses assignment controls for that event, and the dashboard clears stale browser state from the former chip handoff. Agent keychain verification and loan-officer public/private chip behavior outside this assignment action are unchanged.
- `[IMPLEMENTED]` Every live-event, scheduled-visit, confirmed-outreach, and automatic loan-officer assignment is constrained to an active `agent_loan_officer_relationships` row for the hosting agent. COMMAND dropdowns show only those linked loan officers, and the server rejects forged or stale unlinked selections.
- `[IMPLEMENTED]` The agent event dashboard no longer links into `/lo-affordability-guidance` or exposes property-fit inputs, loan limits, rate assumptions, or unreviewed scenarios. It can text the assigned loan officer for help and can read only a limited outcome after the loan officer marks a scenario reviewed.
- `[IMPLEMENTED]` `/lo-affordability-guidance` and the private affordability write actions require a signed-in Supabase loan-officer session whose email matches the requested active verified profile. A URL loan-officer UID is no longer authorization.

## 2026-07-29: Owner-authorized toll-free outreach run

- `[IMPLEMENTED]` With explicit owner authorization, production temporarily raised `OUTREACH_MAX_OPT_OUT_RATE` from 1% to 15% while retaining the 5/run, 20/rolling-hour, and 100/rolling-24-hour caps plus quiet hours, suppression, duplicate-phone cooldown, and future-event eligibility. Four sequential runs sent 20 eligible messages through the dedicated Twilio outreach route; one duplicate candidate was safely skipped and replaced, with no send failures.
- `[IMPLEMENTED]` The 1% health threshold was restored immediately after the first 20-message portion and verified live. The owner subsequently expanded the authorized run from 25 to 40 total; the remaining 20 are scheduled as four capped 5-message runs at 8:45 PM ET, after the rolling-hour window reopens and before quiet hours, with a required preflight and immediate restoration to 1%.

## 2026-07-29: Admin open-house controls

- `[IMPLEMENTED]` REL8TION COMMAND now separates Open Houses from Signs. The Open events and Confirmed OH overview cards route to the dedicated Open Houses workspace instead of loading sign inventory first.
- `[IMPLEMENTED]` The Open Houses workspace shows live events first, provides direct event-dashboard links, keeps recent ended events in a separate history section, and lists scheduled/confirmed coverage appointments with LO assignment controls.
- `[IMPLEMENTED]` Admins can cancel a scheduled open house after typing `CANCEL`. Cancellation preserves buyer/event/outreach history, releases visit participants and linked availability blocks, returns accepted outreach to interested status, and ends any matching live event through the existing sign/LO cleanup path.

## 2026-07-23: REL8TION COMMAND loader recovery

- `[IMPLEMENTED]` `/admin` now hands off directly to the full REL8TION COMMAND document instead of asynchronously fetching it and injecting it with `document.write`. This prevents upstream HTML error pages from appearing as text and preserves the dashboard controls when the wrapper route is used.
- `[IMPLEMENTED]` The live Manual SMS Backup page and its canonical app copy now keep a prominent, sticky **Back to REL8TION COMMAND** control visible on desktop and mobile.
- `[IMPLEMENTED]` REL8TION COMMAND now excludes Android inbound messages unless the sender resolves to an outreach queue row with an agent identity. Raw/unmatched Android texts remain stored for diagnostics but no longer appear in the dashboard inbox, counts, latest-inbound cards, or direct thread detail.
...
## 2026-07-24: Owner-authorized toll-free outreach run

- `[IMPLEMENTED]` With explicit owner authorization, the sender temporarily used a 28/hour ceiling and 10% health threshold to process 28 eligible future-event outreach attempts through the toll-free Twilio route. Twenty-six messages were accepted by Twilio; 11 rows were skipped by existing duplicate or opt-out protections, with no send failures.
- `[IMPLEMENTED]` Immediately after the run, production returned to 5/run, 20/hour, and 100/rolling-24-hours with the 1% rolling opt-out health gate. The final no-send diagnostic reported zero eligible candidates, 26 sends in the previous hour, 28 in the previous 24 hours, and `health_blocked=true` at 18 opt-outs out of 225 seven-day sends (8.0%).

## 2026-07-22: Admin Event Pass Freshener

- `[IMPLEMENTED]` REL8TION COMMAND's Signs and Events area now has an admin-only Freshen Event Pass scanner. It can use the rear camera, a QR image, or a pasted printed code/QR URL and performs one immediate reset before stopping the camera.
- `[IMPLEMENTED]` Freshening ends the pass's active event and live loan-officer coverage, clears the old host/sign link, unclaims only an NFC row explicitly marked `event_pass_keychain`, clears duplicate Event Pass inventory aliases tied to the same backing sign, and cancels pending activation sessions. Event history, check-ins, disclosures, printed public codes, sponsor assignment/configuration, and reuse settings are preserved.
- `[IMPLEMENTED]` Fresh Event Pass inventory no longer falls through to `smart_signs.public_code` legacy lookup, preventing a reset QR from silently reconnecting to an old host's orphaned sign. Legacy fallback remains available for non-Event-Pass Smart Sign inventory.
- `[NEEDS VERIFICATION]` Deploy the admin scanner/API changes, test on a designated reusable Event Pass, and verify the complete QR -> new host NFC -> listing activation sequence before relying on it in the field.

## 2026-07-22: Loan-officer open-house contact cards

- `[IMPLEMENTED]` The loan-officer dashboard's Next Open Houses cards now hydrate missing agent contact fields from the linked outreach queue instead of allowing blank visit fields to overwrite that context. Each card explicitly shows the hosting agent's name, company/brokerage, and phone, with Call, Text, and Directions actions.

## 2026-07-22: Outreach rolling daily ceiling

- `[IMPLEMENTED]` With explicit owner approval, the production `send-agent-outreach` rolling 24-hour ceiling is 150 messages. The sender remains limited to 5 per run and 20 per rolling hour.
- `[IMPLEMENTED]` Production `OUTREACH_MAX_OPT_OUT_RATE` is corrected to `0.01`, which the sender interprets as 1%. A post-deploy no-send diagnostic reported 277 seven-day outreach sends, 23 opt-outs (8.30%), and `health_blocked=true`, so automatic sends remain stopped even when rolling hourly/daily capacity is available.
- `[IMPLEMENTED]` At owner direction, outreach operator mode is `live`/manual. All 81 unsent future automatic rows were moved to `send_mode=manual`; combined with the existing manual row, production has 82 future manual rows (57 `manual_ready`, 25 awaiting mockups) and zero unsent future automatic rows. A no-send diagnostic confirmed `candidate_rows=0`.

## 2026-07-21: Marketing homepage website-builder offering

- `[IMPLEMENTED]` The user-provided current REL8TION homepage markup now replaces the older tracked `home.html`. The current design includes the Agent Website Builder in the primary offering copy, adds a dedicated offering card, and links both the primary menu and footer directly to `https://my.rel8tion.me`.
- `[NEEDS VERIFICATION]` `home.html` is a WordPress-side source-tracking file and is not automatically synchronized to the live `rel8tion.info` homepage. Publish the updated markup in WordPress, then verify the live desktop and mobile navigation and builder links.
...
- `[IMPLEMENTED]` As of 2026-07-22, production `send-agent-outreach` caps are 5/run, 20/hour, and 150 per rolling 24 hours. The rolling opt-out health gate uses a 7-day window, 20-send minimum, and corrected 1% maximum rate; future-event eligibility and old/manual/past-event backlog exclusions remain in force.
...
- `[IMPLEMENTED 2026-08-02]` Agent Ranking labels `active_listing_count` as a ListReports-reported snapshot instead of presenting it as the number of REL8TION listing rows. Search results also show a separate database-current count, and clicking an agent separates active/pending listing inventory from upcoming matched open houses. The production two-hour `sync-agent-listing-inventory` cron scans active/pending OneKey records and matches ranked agents, claimed `agents`, and outreach rows marked `interested`, `confirmed_open_house`, `accepted_open_house`, or `drip_scheduled` by contact or exact name/brokerage. Ranked-only agents are included for listing visibility; future listing marketing is server-limited to claimed/worked-with or positive-interest relationships. The cron fails closed without `CRON_SECRET`, and production currently has both `AGENT_LISTING_INVENTORY_ENABLED=true` and `AGENT_LISTING_INVENTORY_PROMOTE_OPEN_HOUSES=true`, so upcoming matched inventory is also upserted into `open_houses`. A manual production invocation after enabling promotion wrote 77 OneKey rows at 2026-08-02 06:57 UTC; a follow-up read confirmed every currently upcoming inventory listing had a matching `open_houses` row, including all 51 listings associated with `worked_with` agents. Specific upcoming-open-house outreach remains a separate independently gated action behind `AGENT_LISTING_INVENTORY_OUTREACH_ENABLED=true`, always stages manual/non-sending outreach, and was not changed as part of this activation. A read-only production query on 2026-07-29 confirmed the earlier Ruth Chalco mismatch: the July 26 ranking row reported `active_listing_count=3`, while `matched_open_house_count=0`; the only Ruth row in `open_houses`/`listing_agents` was 126 Scranton Ave with an open house that ended 2026-06-07, and no Ruth `agent_websites`/active `agent_website_listings` row existed.
...
- `[IMPLEMENTED]` Buyer affordability guidance entry and property testing are loan-officer-only operations. The host-agent event view receives only guidance-presence metadata plus reviewed outcome labels/notes; it never receives the LO's private cap, rate, term, down-payment, insurance, or rent assumptions.
...
- `[IMPLEMENTED]` Root cron code includes outreach generation and send endpoints. As of 2026-07-22, the Vercel cron and `send-agent-outreach` Edge Function hard-cap automatic sends at 5 per run, 20 per rolling hour, and 150 per rolling 24 hours. Automatic initial sends do not require `approved_for_send=true`; eligible rows are `send_mode=automatic`, generated, rendered, due, with a listing photo and pending initial SMS copy, and all health/suppression gates still apply.
...
- `[NEEDS VERIFICATION]` Migration `20260730012601_agent_relationship_listing_inventory.sql` adds service-only `agent_listing_inventory` with RLS, explicit anon/authenticated revocation, service-role grants, relationship/listing uniqueness, current-state retention, and agent/open-house lookup indexes. It is checked in but has not been applied to or advisor-checked against REL8TION production from this session.
- `[VERIFIED PREVIEW 2026-07-29]` The listing-inventory migration was applied only to Supabase preview branch `agent-listing-inventory-test-20260729` (`zzjbvenalpjtfaljbepc`), never to production. Because the production migration ledger is drifted, the branch did not contain the Agent Rankings tables; a test-only fixture under `test/fixtures/agent-ranking-branch-fixture.sql` creates only the minimal service-role ranking/upload tables and one clearly labeled Ruth Chalco row. The real `api/admin/agent-ranking.js` path was then exercised locally against that branch: ListReports reported `3`, protected database inventory returned `1`, one upcoming open house loaded in the profile, listing outreach remained disabled, RLS/role checks passed, and Supabase security/performance advisors reported no issues. The preview branch incurs its displayed hourly charge until it is deleted.
