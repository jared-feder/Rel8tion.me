...
- `[PARTIAL]` `/open-house-kit` is a separate Event Pass upgrade page with Open House Kit features/benefits and three closeout choices: buy the one-time kit, activate a monthly Event Pass/dashboard subscription, or call the sponsoring loan officer for next-week setup. The browser page does not collect raw card data; `/api/checkout/open-house-kit` uses Stripe Checkout, defaults the one-time kit to price `price_1TYtd12LIj1DZULXtTeeeYSm`, and supports monthly checkout through `STRIPE_EVENT_PASS_MONTHLY_PRICE_ID` or `STRIPE_EVENT_PASS_MONTHLY_PAYMENT_LINK`. Live Stripe secret/monthly env vars and product pricing still need verification.
