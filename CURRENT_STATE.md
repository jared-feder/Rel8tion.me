...
- `[IMPLEMENTED]` REL8TION COMMAND overview stat cards are clickable navigation controls. The former Event Pass stat/section is labeled as QR codes in the admin UI because those rows are printed pass QR inventory, not a separate operational count that needs Event Pass wording.
