...
- `[IMPLEMENTED]` Outreach can override the default outreach provider by brokerage with `SMS_TWILIO_OUTREACH_BROKERAGES`; as of 2026-06-24 this is used for Douglas Elliman so those outreach rows send through Twilio/MMS while general outreach remains on Android.
- `[IMPLEMENTED]` Root cron code includes outreach generation and send endpoints. Outreach sending is throttled for Android SMS Gateway safety: the send cron defaults to 3 per run, and the `send-agent-outreach` Edge Function hard-caps automatic sends with `OUTREACH_SEND_MAX_PER_RUN` defaulting to 3 and `OUTREACH_SEND_MAX_PER_HOUR` defaulting to 6. Automatic initial and follow-up sends require `approved_for_send=true`.
- `[IMPLEMENTED]` REL8TION COMMAND surfaces generated/rendered due outreach rows that still need `approved_for_send=true`, with explicit Approve send and Hold send controls.
...
- `[INTENDED]` To protect Twilio from outreach-volume flagging, production outreach should use `SMS_OUTREACH_PROVIDER=android_gateway` while event/owner/system texts stay on `SMS_EVENTS_PROVIDER=twilio`.
