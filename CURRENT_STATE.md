...
- `[PARTIAL]` `/loan-officer-dashboard` now renders a permanent LO owner-dashboard preview modeled on `/agent-home`, with verified profile identity, live/upcoming coverage, inline add/remove availability management, worked-agent outreach creation, and a financing-help inbox with thread history and buyer reply sending. The outreach composer creates personalized Event Pass, remote-coverage, or in-person-coverage copy and opens the LO's SMS/email client; it does not auto-send. Availability captures an exact start/end, service ZIP, and coverage radius and feeds the existing field assignment records. Preview-host writes require explicit confirmation because they use production data and messaging infrastructure. `/lo-field-dashboard` remains a backward-compatible detailed-operations alias. Authenticated ownership, automatic financing-request conversation creation, saved outreach records, and recurring weekly schedule templates remain to be built.
...
- `[IMPLEMENTED]` Root cron code includes outreach generation and send endpoints. As of 2026-07-17, production `send-agent-outreach` caps are 5 per run, 20 per rolling hour, and 100 per rolling 24 hours. Automatic initial sends do not require `approved_for_send=true`; eligible rows are `send_mode=automatic`, generated, rendered, due, with a listing photo and pending initial SMS copy. Candidate selection remains limited to future open houses; do not release old, manual, or past-event backlog rows.
