...
- `[VERIFIED]` The focused Event Pass reuse, membership/NFC, private-dashboard security, product-ladder, pricing, browser-script parse, and route-map checks pass locally. Production code, actual recap delivery, real Stripe payment-mode Checkout, webhook/fulfillment creation, physical NFC routing, mobile rendering, live SHA, and deployment state remain `[NEEDS VERIFICATION]`; this section is not a production-release claim.
