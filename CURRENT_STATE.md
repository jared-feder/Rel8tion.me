...
- `[IMPLEMENTED]` Buyer check-in calls `send-lead-sms` for buyer and agent SMS. When the linked `open_houses.link` or event setup-context listing URL is available, the buyer confirmation SMS includes a short `/l/<open_house_id-or-event_id>` REL8TION link that redirects through `api/open-house-link.js` to the saved MLS/listing page. Check-in metadata stores both the original listing URL and short REL8TION URL. Local `send-lead-sms` source is now checked in at `supabase/functions/send-lead-sms/index.ts`; the function is user-reported as active and working in Supabase, while deployed source/version matching still needs dashboard verification.
...
- `[IMPLEMENTED]` Buyer check-in confirmation SMS now includes a short REL8TION listing link when a linked MLS/listing URL is available. `/l/<open_house_id-or-event_id>` resolves server-side through `api/open-house-link.js`, validates the stored URL, and redirects to the saved listing page.
