...
- `[IMPLEMENTED]` `/admin` and `/admin.html` now enter through a server-side authorization route on this branch. Platform administrators require `platform.admin`; legacy COMMAND NFC UID/token access is preserved and redirects deliberately to `/command`.
...
- `[RISK]` Vercel Preview for `rel8tion-me` receives the production-connected `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY`. Preview verification for this feature must remain read-only and must not create test memberships, assignments, tasks, activity, or audit rows in production.
...
- `[IMPLEMENTED]` `npm run lint`, `npm run typecheck`, `npm test`, and `npm run build` now provide repository-appropriate static syntax, authorization, RBAC, route-contract, and architecture checks for the universal application.
- `[NEEDS VERIFICATION]` This feature is not production-deployed. A protected Vercel Preview must be generated from the feature branch, visually verified at mobile and desktop breakpoints, and checked against all permanent public routes before any merge decision.
