...
- `[PARTIAL]` `/open-house-kit` is a separate Event Pass upgrade page with Open House Kit features/benefits and a secure payment handoff. The browser page does not collect raw card data; `/api/checkout/open-house-kit` either redirects to `STRIPE_OPEN_HOUSE_KIT_PAYMENT_LINK`/`OPEN_HOUSE_KIT_PAYMENT_LINK` or creates a Stripe Checkout Session from `STRIPE_SECRET_KEY` plus `STRIPE_OPEN_HOUSE_KIT_PRICE_ID`/`OPEN_HOUSE_KIT_PRICE_ID`. Live Stripe env vars and product pricing still need verification.
