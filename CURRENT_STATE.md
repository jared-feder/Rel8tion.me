...
- `[PARTIAL]` `/loan-officer-dashboard` now renders a permanent LO owner-dashboard preview modeled on `/agent-home`, with verified profile identity, live/upcoming coverage, inline add/remove availability management, a simplified My Agents section, and a financing-help inbox with thread history and buyer reply sending. Explicit buyer financing consent at event check-in creates an idempotent inbox conversation; manual LO assignment/reassignment attaches every open event conversation to the assigned profile. My Agents has one automatic support-offer action for established agent relationships, combining Event Pass and remote live-financing coverage in approved copy; SMS sends immediately and email sends when configured. Availability captures an exact start/end, service ZIP, and coverage radius and feeds the existing field assignment records. Preview-host writes require explicit confirmation because they use production data and messaging infrastructure. `/lo-field-dashboard` remains a backward-compatible detailed-operations alias. Authenticated ownership, durable support-offer delivery logs, and recurring weekly schedule templates remain to be built.
- `[PARTIAL]` Real `/loan-officer-dashboard` access now requires a Supabase passwordless email session whose email matches the approved verified profile. Automatic My Agents sending and LO-to-buyer chat replies repeat that ownership check server-side; demo mode remains local and non-sending. Availability and other older field-operation APIs still need the same ownership boundary before the full dashboard can be promoted publicly.
- `[NEEDS VERIFICATION]` Supabase Auth magic-link email delivery and allowed redirect URLs must be tested with an approved LO email on the branch preview before production promotion.
...
- `[IMPLEMENTED]` Root cron code includes outreach generation and send endpoints. As of 2026-07-17, the production Edge Function allows 15 per direct run, 20 per rolling hour, and 80 per rolling 24 hours; the root Vercel cron continues to request at most 5 per invocation. Automatic initial sends do not require `approved_for_send=true`; eligible rows are `send_mode=automatic`, generated, rendered, due, with a listing photo and pending initial SMS copy. Candidate selection excludes same-day open houses and prioritizes late Saturday, then Sunday, before other future events; old, manual, and past-event backlog rows remain excluded.
- `[RISK]` The owner temporarily set `OUTREACH_MAX_OPT_OUT_RATE=1` on 2026-07-17, so the opt-out health gate does not currently stop outreach. Restore the approved threshold after the requested sending window and review replies before further volume changes.
...
- `[IMPLEMENTED]` Confirming an outreach open house creates a scheduled field visit even when no live `open_house_events` row exists. REL8TION COMMAND now exposes LO assignment directly on that Confirmed Open Houses row, creates the primary financing-support participant, and sends the same appointment confirmations without incorrectly marking the future event live.
