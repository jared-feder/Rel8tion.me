...
- `[VERIFIED LOCAL]` At the reported 430×932 layout, the corrected prompt remained fixed at `top=844`, `bottom=922` while the first name field was below the fold at `top=1154`. Tapping it hid the prompt and brought **Begin Your Check-In** to `top=148`. The nine-case mobile, seven-case security, app, route, module-syntax, and diff checks pass. Production deployment and live tall-phone verification remain pending.
