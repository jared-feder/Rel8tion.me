...
- `[IMPLEMENTED 2026-08-18]` Migration `20260818185810_lock_backend_only_security_surfaces.sql` is the first phased security-advisor repair. It forces RLS and removes `anon`/`authenticated` CRUD from the two historical open-house backups, `open_house_agents`, `open_house_dates`, `open_house_photos`, `buyer_agent_links`, the two OneKey member storage tables, and `agent_outreach_log`. It also converts the three operational outreach/OneKey views to security-invoker, makes them service-role-only, and removes browser execution from the two outreach queue-staging RPCs while retaining service-role execution.
- `[VERIFIED 2026-08-18]` The exact migration was applied individually to production project `nicanqrfqlbnlmnoernb` as migration version `20260818190154`. Live catalog read-back confirmed forced RLS, no browser CRUD, and service-role CRUD on all nine tables; security-invoker/service-only access on all three views; fixed `pg_catalog, public` search paths and service-only execution on both queue RPCs. Anonymous PostgREST probes return `401` for every locked table/view while the intentionally public `open_houses` compatibility lane still returns `200`. Security-advisor errors fell from 21 to 9. A disposable PostgreSQL 18.4 execution, 23 focused security/outreach/Event Pass/activation/loan-officer checks, route-map verification, and the complete production route check passed.
- `[IMPLEMENTED 2026-08-18]` Migration `20260818190418_lock_public_catalog_writes.sql` preserves anonymous/authenticated SELECT on the browser-required `brokerages`, `open_houses`, and `listing_agents` catalogs through explicit RLS policies while removing browser insert/update/delete and retaining service-role CRUD.
- `[VERIFIED 2026-08-18]` The public-catalog migration was applied as production version `20260818190804`. Live policy/grant read-back passed, anonymous zero-row PostgREST probes remained HTTP `200` for all three catalogs, and the full production route suite passed. Security-advisor RLS-disabled errors fell from 9 to 6, and from 21 to 6 across both phases. The first attempt included PostGIS `spatial_ref_sys`, but the migration transaction safely rolled back because that extension-managed table is owned by `supabase_admin`; it was removed from the reviewed business-table migration and no partial changes remained.
- `[IMPLEMENTED 2026-08-18]` Migration `20260818190921_lock_scan_events.sql` forces RLS and removes all browser-role access from legacy `smart_sign_scan_events`, retaining service-role CRUD. Current routed `/k` and `/sign-demo-activate` source is guarded against this legacy table dependency.
- `[VERIFIED 2026-08-18]` The scan-event migration was applied as production version `20260818190948`; live read-back confirmed no `anon`/`authenticated` access and retained service-role CRUD. Security-advisor RLS-disabled errors fell from 6 to 5, and from 21 to 5 across all three phases.
- `[PARTIAL]` The remaining RLS-disabled advisor errors are `event_loan_officer_sessions`, `agent_outreach_queue`, `agent_outreach_replies`, `smart_sign_inventory`, and extension-owned `spatial_ref_sys`. The four REL8TION business tables still have active browser dependencies and must move behind validated server routes or narrow read-only surfaces before broad grants are revoked. `spatial_ref_sys` contains standard PostGIS reference data rather than REL8TION customer/operational data and requires an extension-owner/admin path if its advisor notice is to be removed.
