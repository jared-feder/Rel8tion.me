...
- `[PARTIAL]` `/loan-officer-dashboard` now renders a permanent LO owner-dashboard preview modeled on `/agent-home`, with verified profile identity, live/upcoming coverage, inline add/remove availability management, and a financing-help inbox with thread history and buyer reply sending. Availability captures an exact start/end, service ZIP, and coverage radius and feeds the existing field assignment records. Preview-host writes require explicit confirmation because they use production data and messaging infrastructure. `/lo-field-dashboard` remains a backward-compatible detailed-operations alias. Authenticated ownership, automatic financing-request conversation creation, and recurring weekly schedule templates remain to be built.
