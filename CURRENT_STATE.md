...
- `[IMPLEMENTED]` Sponsored closeout now server-sends an idempotent event recap to the agent profile email through the configured SMTP provider or Resend fallback. The email contains the event totals, check-in contact rows, and disclosure completion status without attaching or deleting the underlying legal evidence. Delivery status is retained in `setup_context.event_recap_email`; a provider or missing-address failure does not prevent safe event closeout and is shown to the agent.
