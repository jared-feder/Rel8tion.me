...
- `[PARTIAL]` Root `vercel.json` now includes Vercel Cron entries for `/api/cron/refresh-open-house-data`, `/api/cron/generate-agent-outreach`, `/api/cron/render-agent-mockups`, and `/api/cron/send-agent-outreach`. The outreach cron routes require `CRON_SECRET`; generation/render/send execution, Vercel dashboard state, and Twilio behavior remain `[NEEDS VERIFICATION]` until deployed and checked.
...
- `[NEEDS VERIFICATION]` New `field_coverage_availability` schema and assignment columns are additive repo migration code only until applied and verified live. Distance ranking is exact only when lat/lng data exists; otherwise it falls back to ZIP proximity.
- `[PARTIAL]` `event_conversations` / `event_conversation_messages` migration was applied to linked Supabase and table existence was verified by SQL query on 2026-05-12. RLS policy posture, API route deployment, and full browser behavior still need verification.
...
- `[NEEDS VERIFICATION]` Vercel Cron dashboard/API state needs to be rechecked after deploying the branch with the updated `vercel.json`; previous inspections were from older deployments and did not include the new outreach cron definitions.
...
- `[IMPLEMENTED]` Event conversation files were added: SQL migration for `event_conversations` / `event_conversation_messages`, serverless `/api/event-chat/*` helpers, LO/field dashboard chat controls, agent lead-card chat controls, and buyer post-check-in chat composer.
- `[IMPLEMENTED]` Field availability files were added: SQL migration for `field_coverage_availability`, `/api/field-demo/availability`, `/api/field-demo/available-support`, field-dashboard availability controls, and outreach-to-availability scheduling.
