...
- `[IMPLEMENTED]` Sponsored activation writes `event_access_mode=sponsored_event_only`, `agent_dashboard_retention=event_only`, and `event_recap_delivery=email` into the server-created event context. `/api/agent-event-dashboard` preserves the event/check-in rows for compliance and audit, filters those sponsored event-only rows from an unpaid agent's permanent history, and blocks reopening an ended sponsored dashboard unless active membership is verified. Paid members may retain and reopen history.
- `[IMPLEMENTED]` Sponsored closeout now loads the authoritative check-ins and server-sends an idempotent event recap to the agent profile email through the configured SMTP provider or Resend fallback before ending the event. The email contains the event totals, check-in contact rows, and disclosure completion status without attaching or deleting the underlying legal evidence. A check-in query, recipient, provider, or delivery failure leaves the event active for correction and retry; a recorded `setup_context.event_recap_email.status=sent` is reused so a later close retry does not send a second copy.
- `[IMPLEMENTED]` Product copy no longer says an Event Pass becomes a Rel8tionChip. REL8TION Agent remains the recurring software/history membership. The reusable Smart Sign and custom company-branded Rel8tionChips are the separate `$199` one-time Open House Kit hardware upsell. The new `open_house_kit_member_hardware` Checkout uses only the existing one-time kit line item, creates no subscription data, and requires a current server-verified claimed NFC session plus active agent membership so it cannot add a second membership.
- `[IMPLEMENTED]` The ended-event dashboard gates that `$199` member hardware offer on `membership_active=true`. Sponsored event-only and unpaid normal Event Pass users instead see the `$29/month` REL8TION Agent activation path and no permanent-history promise or member-only kit price.
- `[IMPLEMENTED]` The `$29` Event Pass return now explains that the verified agent already has a starter REL8TION profile, activates the preserved selected open house first, and then offers **Complete My Profile** plus a digital-card preview. `/claim?edit=profile` now opens the existing profile editor for a claimed pass instead of redirecting past it; saving updates the same agent slug and accepts only bounded same-origin returns to `/agent-dashboard` or `/agent-home`, preserving the live event.
- `[VERIFIED]` PR `#46` merged the reviewed hotfix commit `a2ddc39b` into `main` as `3ac06b79`. Vercel Git production deployment `dpl_9wdYMfMVih4enDcdg3eqwwmD3BU4` is `READY`, targets production, carries the `app.rel8tion.me` aliases, and reports that exact merge SHA. The prior production rollback candidate is `dpl_CzctEzuXz4W4V29pGK1p3AMZtmAw` at `9316fd3f`.
- `[VERIFIED]` The eight-case product-ladder suite, eight-case reuse suite, seven-case private-dashboard security suite, nine-case membership/NFC suite, eight-case activation/search suite, nine-case mobile-viewport suite, pricing verifier, route-map verifier, app-architecture verifier, affected server syntax checks, and diff check pass from a clean worktree at the deployed SHA. GitHub Branch Safety, Repo Checks, Vercel Agent Review, and Vercel preview checks also passed.
- `[VERIFIED]` The production route suite passes for `https://app.rel8tion.me`; the exact rewritten Event Pass reuse, agent dashboard, claim, kit intake, and public-pricing routes return `200`, while an unauthenticated private-dashboard probe returns the expected `401`. The live rewritten Event Pass HTML matches the deployed-SHA source after newline normalization. A bounded post-deploy scan found zero `5xx` responses on the deployment and only the existing Node `url.parse()` deprecation warning in grouped runtime errors.
- `[VERIFIED]` At a `320x568` mobile viewport, the exact deployed Event Pass source renders the sponsored event-only option before the paid permanent-record option, shows `$29/month`, keeps both actions full-width, has no horizontal overflow, and enables the sponsor action only after consent. The live no-context route renders the expected recovery state and support action. The mobile option test used a read-only fixture response and did not activate an event or start Checkout.
- `[NEEDS VERIFICATION]` Actual sponsored recap delivery through the configured production email provider, a real Stripe payment-mode member-hardware Checkout plus signed webhook/fulfillment lifecycle, and a physical NFC activation/closeout were not exercised during this release because each would create a real external or field side effect. Code, route, deployment, mobile, and rollback evidence are production-verified; provider delivery and physical-device behavior remain separate prerequisites for an end-to-end live transaction claim.
- `[VERIFIED]` `AGENTS.md reviewed - no durable rule change`
...
- `[VERIFIED]` Live investigation of `ep-00b289ccff24` found three phone activation attempts returning `403` before the membership/sponsor branch. Its NFC is currently claimed to Amy Kernaghan SFR, and the inventory/backing sign remain inactive and unassigned. A controlled validation using Amy's verified 41 Chelsea Place listing returned the expected `402` membership-required response, proving the released sponsor/$29 choice flow works and the phone attempts had submitted a listing the server could not authorize for Amy. The physical retry on the correct listing remains pending.
- `[IMPLEMENTED]` A membership-required normal Event Pass reuse now leaves the activation form and opens the dedicated full-screen `/event-pass-reuse` page with the already selected pass, claimed agent, listing, and listing-agent/same-brokerage authorization context. The old small warning and instruction to tap the same NFC again are removed from this error path, eliminating the reported return-to-the-same-page loop.
- `[IMPLEMENTED]` The first choice presents the server-resolved active loan officer with their photo, name, and company and explains that the loan officer will sponsor the open house, that the agent should thank them and maintain the relationship, and that this is limited REL8TION access. Continuing requires explicit per-event host consent; the server independently re-resolves the assigned loan officer, rejects a client-supplied mismatch, records `event_pass_coverage_consents`, and creates the live loan-officer session before making the pass live.
- `[IMPLEMENTED]` Reuse sponsor precedence now starts with the loan officer assigned to the pass's earliest `open_house_events` row, using the historical `event_loan_officer_sessions` assignment or preserved sponsor identifiers in `setup_context`. If that initial event has no usable active loan-officer profile, both the choice-page resolver and the activation-time resolver fall back to Brian Puls's canonical active production profile (`verified_profiles.uid=7e05fcf5-18de-4ba1-b689-b944602ed4ca`). The client still cannot choose or substitute the sponsor.
- `[VERIFIED]` A read-only production check confirmed Brian Puls's canonical profile is active. Event Pass `ep-00b289ccff24` is fresh/unlinked in `smart_sign_inventory` but retains its recoverable inactive backing `smart_signs` row and earliest ended event; that event has neither a loan-officer session nor preserved sponsor identifiers, so this pass will take the Brian Puls fallback after the follow-up release is live.
- `[IMPLEMENTED]` The second choice uses the catalog-controlled REL8TION Agent monthly price of `$29/month`. As of the 2026-08-19 product-ladder correction, it explains permanent software/history access and no longer claims that the Event Pass transforms into a Rel8tionChip; branded Rel8tionChip/Smart Sign hardware is a separate one-time Open House Kit purchase.
- `[IMPLEMENTED]` Event Pass checkout now stores a bounded same-origin return path in Stripe metadata. A verified browser return resumes the exact interrupted `/event-pass-reuse` context with active membership instead of redirecting to `/agent-home`; cancellation preserves the same registration context.
- `[VERIFIED]` PR `#38` merged the choice-page/NFC-loop repair as `39368226`; Vercel production deployment `dpl_7tgP4k2d4HBwH1EymJXVoPHaGbnd` is READY/PROMOTED from that exact `main` SHA. The full production route suite passed, live source checks confirmed the sponsor-first page and NFC redirect, and the public pricing catalog returns `2900` cents monthly. The focused eight-case reuse suite, seven-case activation suite, seven-case security suite, server syntax check, app architecture check, and route-map check pass for the Brian Puls fallback; it remains `[NEEDS VERIFICATION]` in production until reviewed, merged, and checked on a new exact-SHA deployment. A physical NFC retry also remains pending.
...
- `[VERIFIED]` The focused three-case admin Event Pass management suite, seven-case activation suite, app architecture check, route-map check, and server-module syntax checks pass locally. PR `#37` merged this repair to `main` as `b661c0ea`; its live production behavior is rechecked with the release evidence for the Event Pass reuse choice-page change above.
...
- `[VERIFIED]` Production repair detached every proven first-and-last-name-conflicting agent link while retaining each prior UUID, canonical name, reason, and repair timestamp in the row's JSON audit metadata. Read-back found zero remaining name-conflicting links in `agent_rankings`, `agent_listing_inventory`, and `agent_production_import_rows`. No agent, listing, import, outreach, or event-history row was deleted.
- `[VERIFIED]` Lisa Luttinger's ranking remains linked only to her UUID; Lisa Pellegrino's ranking is now unlinked pending a person-specific canonical match. No listing inventory remains attached to Luttinger's UUID under another agent name.
- `[VERIFIED]` The retained `109 Henrietta Ave, Oceanside` source row has no agent attached, and Teresa A. DeDonato's retained inventory contains only Massapequa Park, East Meadow, and Massapequa properties. The exact prior COMMAND screen association is no longer retained, but it is consistent with the now-confirmed systemic identity merge rather than evidence that Teresa listed Henrietta Avenue.
- `[VERIFIED]` Perry Pappas exposed the companion photo failure: a shared Signature office phone caused the same Perry-associated image URL to appear on Perry plus David W. Holmes, Jill Capozzi, Marlene Gross, Rosa DellAmore, and Tina Rossetti rows. That image was rejected and removed from display. A full cross-source audit found 25 photo URLs attached to incompatible person names; 151 ambiguous outreach, relationship, and listing-agent photo references were quarantined, while eight references matching a manually verified same-name owner were preserved. Post-repair verification found no Perry asset references and no photo URL still shared across incompatible names.
- `[IMPLEMENTED]` COMMAND profile aggregation, ListReports import matching, relationship/history matching, listing-inventory sync, manual photo upload, and OneKey headshot enrichment now enforce compatible first-and-last-name identity before accepting a UUID, explicit row id, phone, email, or fuzzy match. Shared office phones produce separate relationship keys. Harmless middle initials and credential suffixes remain compatible.
- `[VERIFIED]` Focused regression suites cover Lisa/Pellegrino stale UUIDs, Teresa name variants, Perry/shared-office phones, explicit photo row IDs, imports, inventory relationships, history, prospect/profile behavior, and headshot propagation. All 78 Node test cases plus the ranking identity verifier pass; deployment verification is recorded with the release evidence below.
- `[VERIFIED]` Hotfix PR `#31` merged commit `9e108c6d` into `main` as `35fc553c`. Vercel production deployment `dpl_HuVPiYimozLN2g4XRScnDjX4uc3a` is READY from that exact merge SHA with the production aliases attached. The full production route suite passed, `/api/admin/dashboard` retained its expected unauthenticated `401` boundary, and the bounded post-release error scan found only the existing Node `url.parse()` deprecation warning with no functional COMMAND identity failure.
...
- `[IMPLEMENTED]` Check-in discovery no longer depends on a Tailwind mobile breakpoint or on the floating overlay alone. The page now renders one permanent in-flow **Start Check-In** action directly below the property/host card plus the fixed bottom prompt, and both actions share the same jump-to-check-in behavior. The fixed action is constrained to a 430px centered width but is not hidden when a phone reports a desktop-sized CSS viewport. It starts visible unconditionally instead of running a premature position check before Tailwind finishes styling the live page; subsequent scrolling still hides it when the first field is actually usable.
- `[VERIFIED]` On a fresh 430×932 local reload, the permanent action was visible at `top=556`, `bottom=616` and the fixed action at `top=853`, `bottom=913`. Commit `4a6f96b7` is live through direct Vercel production deployment `dpl_HWrTzA5kBZKbPN7J1aqnGpWSsxYn`, which includes the newer loan-officer hotfix already on `main`; exact SHA, READY/PROMOTED state, and all production aliases were verified. A brand-new live Event Pass QR tab rendered both **Start Check-In** actions in the first viewport, and the permanent action moved **Begin Your Check-In** to `top=84` with all three path choices present. The full production route suite passed, no check-in was submitted, and the deployment had no runtime error logs.
...
## 2026-08-17: Paying-agent Event Pass idle-owner flow (product wording superseded 2026-08-19)

- `[IMPLEMENTED]` A claimed Event Pass has two NFC states. While its open-house event is active, `/k` opens that event dashboard. After the event ends, the same NFC opens `/agent-home` as the membership/owner surface instead of reopening historical event results or immediately forcing another activation. The Event Pass remains Event Pass hardware.
...
- `[IMPLEMENTED]` Permanent dashboard access is gated by an active `pricing_entitlements` REL8TION Agent record containing both `agent_dashboard` and `digital_card`. An unpaid Event Pass owner sees the catalog-controlled membership checkout handoff and public digital card, not private event history. As of 2026-08-19, branded Rel8tionChip and Smart Sign hardware is correctly described as a separate one-time Open House Kit purchase.
