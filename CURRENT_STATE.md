...
- `[PARTIAL 2026-07-30]` Agent Ranking now labels `active_listing_count` as a ListReports-reported snapshot instead of presenting it as the number of REL8TION listing rows. Search results also show a separate database-current count, and clicking an agent separates active/pending listing inventory from upcoming matched open houses. Repo source includes a two-hour `sync-agent-listing-inventory` cron that scans active/pending OneKey records and matches ranked agents, claimed `agents`, and outreach rows marked `interested`, `confirmed_open_house`, `accepted_open_house`, or `drip_scheduled` by contact or exact name/brokerage. Ranked-only agents are included for listing visibility; future listing marketing is server-limited to claimed/worked-with or positive-interest relationships. The cron fails closed without `CRON_SECRET` and remains disabled unless `AGENT_LISTING_INVENTORY_ENABLED=true`. Its default shadow mode writes only protected `agent_listing_inventory` rows; promotion into existing `open_houses` requires the separate `AGENT_LISTING_INVENTORY_PROMOTE_OPEN_HOUSES=true` flag. Specific upcoming-open-house staging also remains hidden and server-blocked until `AGENT_LISTING_INVENTORY_OUTREACH_ENABLED=true`, and always creates manual/non-sending outreach. A read-only production query on 2026-07-29 confirmed the Ruth Chalco mismatch: the July 26 ranking row reports `active_listing_count=3`, while `matched_open_house_count=0`; the only Ruth row in `open_houses`/`listing_agents` was 126 Scranton Ave with an open house that ended 2026-06-07, and no Ruth `agent_websites`/active `agent_website_listings` row existed. The migration, cron deployment, and first production shadow sync remain `[NEEDS VERIFICATION]`.
