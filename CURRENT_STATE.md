...
- `[IMPLEMENTED]` Outreach safety recovery adds a default 30-day same-phone cooldown, a rolling 7-day opt-out health gate (minimum 20 sends, default 1% maximum), a 7-day maximum age for missed-open-house outreach, shorter permission-oriented initial copy, and initial MMS disabled by default. Android remains text-SMS-only; verified Twilio is required for automated outbound MMS.
- `[PARTIAL]` On 2026-07-14, an owner-only MMS through the outreach route was accepted with SID `MM395033030537bb1de5b4b0b6489d7cdd`, one image, and actual sender `+18448211802`, confirming Messaging Service sender-pool selection and outbound MMS submission. Toll-free verification/use-case approval and inbound webhook/reply handling still need verification. Production remains paused for `opt_out_rate_recovery` until the inbound reply test passes.
