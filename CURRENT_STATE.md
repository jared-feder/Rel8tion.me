...
- `[IMPLEMENTED 2026-07-18]` Event Pass activation uses a dedicated mobile `Find My Open House` screen before agent confirmation. Location permission is requested only after the user taps the single locate button, and the screen explains how to enable Safari location access on iPhone when the prompt does not appear. After the agent is confirmed, the selected listing is carried forward to the final activation confirmation without requesting GPS a second time.
