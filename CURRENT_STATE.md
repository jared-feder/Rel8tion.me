...
- `[PARTIAL]` `/open-house-kit` is a separate Event Pass upgrade page with Open House Kit features/benefits and three closeout choices: buy the kit with required monthly service, buy the kit with required paid-for-the-year service, or call the sponsoring loan officer for next-week setup without purchasing. The browser page does not collect raw card data; `/api/checkout/open-house-kit` uses Stripe Checkout in subscription mode, defaults the one-time kit line item to price `price_1TYtd12LIj1DZULXtTeeeYSm`, and adds either `STRIPE_EVENT_PASS_MONTHLY_PRICE_ID` or `STRIPE_EVENT_PASS_ANNUAL_PRICE_ID` as the required recurring service line item. Live Stripe secret/monthly/annual env vars and product pricing still need verification.
