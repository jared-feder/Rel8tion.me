...
- `[IMPLEMENTED]` Check-in discovery no longer depends on a Tailwind mobile breakpoint or on the floating overlay alone. The page now renders one permanent in-flow **Start Check-In** action directly below the property/host card plus the fixed bottom prompt, and both actions share the same jump-to-check-in behavior. The fixed action is constrained to a 430px centered width but is not hidden when a phone reports a desktop-sized CSS viewport.
- `[VERIFIED LOCAL]` At 430×932, the permanent action was visible at `top=556`, `bottom=616` and the fixed action at `top=853`, `bottom=913`, providing two visible calls to action in the reported first-screen layout. The permanent action completed the smooth jump with **Begin Your Check-In** at `top=83`. The nine-case mobile, seven-case security, app, route, module-syntax, and diff checks pass. Production deployment and live dual-action verification remain pending.
