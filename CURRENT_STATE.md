...
- `[VERIFIED LOCAL]` Chrome at 320×568 placed the mobile action at `top=480`, `bottom=558` with the form beginning below the fold at `top=893`. Tapping the action scrolled the check-in section to `top=12`, hid the prompt, and kept the **Begin Your Check-In** heading visible. Buyer and buyer-agent autofill groups rendered with the expected `name`, `tel`, and `email` tokens. The nine-case mobile suite, seven-case Event Pass security suite, app verification, route-map verification, module syntax, and diff checks pass. Production deployment and live-route verification remain pending.
