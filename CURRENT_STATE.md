...
- `[IMPLEMENTED]` `/k` prioritizes sign activation chip scans ahead of backup-keychain linking, so a fresh front/rear sign chip cannot be accidentally claimed as an agent's second keychain while an activation session is waiting for sign chips.
