...
- `[IMPLEMENTED]` Commit `0817386` deployed to Vercel production as `dpl_ChxWLDkbdytfdEap8vCSpFo6Gcfh` on 2026-07-26. The deployment reached `READY`, the standard production route suite passed, and the new relationship route rejected an unauthenticated read with `401` as intended.
- `[NEEDS VERIFICATION]` Complete one authenticated live relationship-board read plus a pin/note/read-back test after the dedicated REL8TION OS credential is added to Vercel.
