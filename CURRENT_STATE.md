...
- `[IMPLEMENTED]` REL8TION COMMAND overview stat cards are clickable navigation controls. The inventory-style Event Pass/QR stat and QR inventory list are no longer emphasized in the admin UI; the overview now surfaces the outreach queue, and the Outreach area shows a queue snapshot with queued records, send status, listing context, and outreach copy.
