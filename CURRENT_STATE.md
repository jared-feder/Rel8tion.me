...
- `[PARTIAL]` Booking reservations use the service-only `rel8tion_call_bookings` ledger defined by `supabase/migrations/20260802140923_rel8tion_call_bookings.sql`. On 2026-08-02, the migration was applied to the linked production project and verified with forced RLS, the confirmed-slot unique index, and no `anon`/`authenticated` table grants. Production Vercel SMTP variables and a sensitive booking IP-hash secret are configured. Route deployment and a designated real-delivery smoke test remain `[NEEDS VERIFICATION]`.
