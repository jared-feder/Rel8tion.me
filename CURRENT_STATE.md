...
- `[VERIFIED LOCAL]` On a fresh 430×932 reload, the permanent action was visible at `top=556`, `bottom=616` and the fixed action at `top=853`, `bottom=913`, providing two visible calls to action in the reported first-screen layout. The permanent action completed the smooth jump with **Begin Your Check-In** at `top=83`. The nine-case mobile, seven-case security, app, route, module-syntax, and diff checks pass. Production deployment and live fresh-load dual-action verification remain pending.
