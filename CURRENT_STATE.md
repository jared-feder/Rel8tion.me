...
- `[PARTIAL]` `/loan-officer-dashboard` now renders a permanent LO owner-dashboard preview modeled on `/agent-home`, with verified profile identity, live/upcoming coverage, inline add/remove availability management, worked-agent outreach creation, and a financing-help inbox with thread history and buyer reply sending. Explicit buyer financing consent at event check-in now creates an idempotent inbox conversation; manual LO assignment/reassignment attaches every open event conversation to the assigned profile. The outreach composer creates personalized Event Pass, remote-coverage, or in-person-coverage copy and opens the LO's SMS/email client; it does not auto-send. Availability captures an exact start/end, service ZIP, and coverage radius and feeds the existing field assignment records. Preview-host writes require explicit confirmation because they use production data and messaging infrastructure. `/lo-field-dashboard` remains a backward-compatible detailed-operations alias. Authenticated ownership, saved outreach records, and recurring weekly schedule templates remain to be built.
