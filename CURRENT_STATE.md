...
- `[IMPLEMENTED 2026-07-18]` Event Pass activation now pauses after the NFC handshake on a dedicated mobile `Find My Open House` screen. Location permission is requested only after the user taps the single locate button, and the screen explains how to enable Safari location access on iPhone when the prompt does not appear.
