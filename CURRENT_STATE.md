...
- `[PARTIAL]` REL8TION COMMAND outreach can now mark a reply/listing as a confirmed true open house. The Reports section builds searchable and sortable upcoming/previous confirmed open house cards and CSV exports with confirmation date, open house times, property details, listing/mockup/agent photo URLs, conversation snapshot, and LO coverage when recorded. Confirmed reports can sort by smart date, open date, confirmed date, loan officer, agent, property, or status.
