...
- Production uses live Supabase and live Twilio settings. Outreach should run through Twilio when `SMS_OUTREACH_PROVIDER=twilio`; use the Android Gateway only as a temporary outreach fallback.
...
- SMS provider routing should use `SMS_PROVIDER=twilio` as the default, `SMS_EVENTS_PROVIDER=twilio` for buyer/event/owner operational texts, and `SMS_OUTREACH_PROVIDER=twilio` for automated/manual outreach. Use `SMS_OUTREACH_PROVIDER=android_gateway` only as a temporary fallback.
- Use `SMS_TWILIO_OUTREACH_BROKERAGES` only for brokerage-specific Twilio/MMS outreach overrides when the default outreach provider is not Twilio.
