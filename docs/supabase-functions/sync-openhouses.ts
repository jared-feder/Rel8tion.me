...
function scoreListingAgent(row: ListingAgentRow): number {
  let score = 0;
...
serve(async (req) => {
  ...
  try {
    ...
    if (listingIds.length > 0) {
      console.log("RESTORING ENRICHED AGENT CONTACTS FROM listing_agents");
      const listingAgents: ListingAgentRow[] = [];

      for (const listingIdChunk of chunkArray(listingIds, 50)) {
        const { data, error: listingAgentsError } = await supabase
          .from("listing_agents")
          .select("open_house_id, name, phone")
          .in("open_house_id", listingIdChunk);

        if (listingAgentsError) {
          console.log("LISTING AGENTS FETCH ERROR:", listingAgentsError);
          throw listingAgentsError;
        }

        if (data?.length) {
          listingAgents.push(...(data as ListingAgentRow[]));
        }
      }

      const bestAgentByOpenHouse = new Map<string, ListingAgentRow & { score: number }>();

      for (const row of listingAgents) {
        if (!row.open_house_id) continue;

        const score = scoreListingAgent(row);
        if (score === 0) continue;

        const current = bestAgentByOpenHouse.get(row.open_house_id);
        if (!current || score > current.score) {
          bestAgentByOpenHouse.set(row.open_house_id, { ...row, score });
        }
      }

      for (const [openHouseId, agentRow] of bestAgentByOpenHouse.entries()) {
        const patch: Record<string, unknown> = {};

        if (agentRow.name?.trim()) patch.agent = agentRow.name.trim();
        if (agentRow.phone?.trim()) patch.agent_phone = agentRow.phone.trim();
        ...
        if (patchError) {
          console.log("OPEN HOUSE AGENT PATCH ERROR:", openHouseId, patchError);
          continue;
        }
