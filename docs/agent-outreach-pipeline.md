...
During opt-out recovery, automatic outreach is hard-capped at 5 sends per run, 10 per rolling hour, and 25 per rolling 24 hours. A 30-day same-phone cooldown and rolling 7-day opt-out health gate apply before delivery, and missed-open-house outreach older than 7 days is skipped. The initial MMS flag is off by code default but explicitly enabled in current production; Twilio receives the generated outreach image first and the NMB business card second.
