...
- Outreach volume risk: the owner-approved ceilings are active at `OUTREACH_SEND_MAX_PER_RUN=7`, `OUTREACH_SEND_MAX_PER_HOUR=20`, and `OUTREACH_SEND_MAX_PER_DAY=150`. Keep STOP/opt-out suppression, duplicate-phone cooldown, provider-health gating, future-event eligibility, and listing/render readiness enabled. Do not raise these ceilings without explicit owner approval and provider health review.
