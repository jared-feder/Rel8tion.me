...
- Date-scoped cron release: upsert `rel8tion_runtime_settings.key='outreach_release_window'` with JSON containing `enabled=true`, `through_open_start`, and `expires_at`. The sender bypasses only the aggregate health stop while the setting is valid and restricts candidates to `open_start < through_open_start`. Disable or expire the setting after the approved date is exhausted.
