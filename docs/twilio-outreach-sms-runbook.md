...
- Automatic outreach is live with `rel8tion_runtime_settings.outreach_send_paused.paused=false` and reason `toll_free_outreach_verified`. Current owner-approved ceilings are 7/run, 20/hour, and 150/day; old manual backlog rows must remain held unless a separately scoped release is explicitly approved.
...
- Outreach volume risk: the owner-approved ceilings are active at `OUTREACH_SEND_MAX_PER_RUN=25`, `OUTREACH_SEND_MAX_PER_HOUR=100`, and `OUTREACH_SEND_MAX_PER_DAY=500`. Keep STOP/opt-out suppression, duplicate-phone cooldown, provider-health gating, future-event eligibility, and listing/render readiness enabled. Do not raise these ceilings without explicit owner approval and provider health review.
- Owner-directed health override: after reviewing current provider failures and opt-out counts, a service-role-authenticated direct invocation may set `override_health_gate=true` for that batch only. This does not clear STOP/opt-out rows, bypass duplicate-phone cooldown, or change the stored health threshold. Record the review and batch result in `CURRENT_STATE.md`.
- Date-scoped cron release: upsert `rel8tion_runtime_settings.key='outreach_release_window'` with JSON containing `enabled=true`, `through_open_start`, and `expires_at`. An optional `from_open_start` adds an inclusive lower bound for a single-date release. The sender bypasses only the aggregate health stop while the setting is valid and restricts candidates to the configured open-start range. Disable or expire the setting after the approved date is exhausted.
