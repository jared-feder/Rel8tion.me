...
async function activateNormalEventPass(body = {}, req = {}) {
  ...
  const dashboardUrl = `/agent-dashboard?event=${encodeURIComponent(event.id)}&agent=${encodeURIComponent(key.agent_slug)}`;
  const profileSetupUrl = `/claim?uid=${encodeURIComponent(uid)}&edit=profile&return_path=${encodeURIComponent(dashboardUrl)}`;
  return {
    event,
    sign,
    ...
    profile_setup_url: profileSetupUrl,
    profile_url: `/b?agent=${encodeURIComponent(key.agent_slug)}`
  };
}
