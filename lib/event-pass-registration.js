...
async function resolveReusableEventPassSponsor({ openHouseId, agentSlug } = {}) {
  const houseId = clean(openHouseId, 300);
  const hostSlug = clean(agentSlug, 160);
  if (!houseId || !hostSlug) return null;

  const visit = await one(
    `field_demo_visits?open_house_id=eq.${enc(houseId)}&agent_slug=eq.${enc(hostSlug)}&status=neq.cancelled&select=*&order=created_at.desc`
  ).catch(() => null);
  if (visit?.id) {
    const participant = await one(
      `field_demo_visit_participants?field_demo_visit_id=eq.${enc(visit.id)}&role=eq.loan_officer&responsibility=eq.financing_support&is_primary=eq.true&status=in.(assigned,confirmed,en_route,on_site,live)&select=*&order=created_at.desc`
    ).catch(() => null);
    const profile = await loadVerifiedLoanOfficer(participant?.participant_profile_id || participant?.participant_uid);
    if (profile) return { profile, source: 'assigned_open_house_coverage', visit, participant };
  }

  const relationship = await one(
    `agent_loan_officer_relationships?agent_slug=eq.${enc(hostSlug)}&status=eq.active&select=*&order=updated_at.desc.nullslast,created_at.desc`
  ).catch(() => null);
  const profile = await loadVerifiedLoanOfficer(relationship?.loan_officer_profile_id);
  return profile ? { profile, source: 'agent_loan_officer_relationship', relationship } : null;
}
...
async function loadEventPassReuseOptions({ uid, agentSlug, publicCode, openHouseId, supportingListingAgent = false } = {}) {
  ...
  const [priorEvent, entitlements, sponsorContext] = await Promise.all([
    ...
    resolveReusableEventPassSponsor({ openHouseId: house.id, agentSlug: key.agent_slug })
  ]);
