...
function recordsMatch(left = {}, right = {}) {
  if (!compatibleAgentNames(left.agent_name || left.name, right.agent_name || right.name)) return false;
...
function relationshipKey(record = {}, agent = null) {
  const nameAnchor = agentNameAnchor(record.agent_name || record.name || agent?.name);
  const agentId = cleanText(record.agent_id || agent?.id);
  if (agentId) return nameAnchor ? `agent:${agentId}|name:${nameAnchor}` : `agent:${agentId}`;
  ...
  if (phone) return nameAnchor ? `phone:${phone}|name:${nameAnchor}` : `phone:${phone}`;
  const email = normalizeEmail(record.email || record.agent_email || agent?.email);
  if (email) return nameAnchor ? `email:${email}|name:${nameAnchor}` : `email:${email}`;
