...
function buildAgentPerformance({
  ...
  for (const row of outreach) {
    ...
    const contacted = Boolean(
      ...
      ['sent', 'delivered', 'manual_text_sent', 'manual_sms_sent', 'twilio_sent'].includes(String(row.initial_send_status || row.send_status || '').toLowerCase())
    );
    profile.outreach_threads.set(String(row.id), {
      ...
      contacted,
      sent_at: row.followup_sent_at || row.initial_sent_at || row.last_outreach_at || null,
  ...
  return profiles
    .map((profile) => {
      ...
      const threads = [...profile.outreach_threads.values()]
        .sort((left, right) => new Date(right.latest_reply_at || right.sent_at || right.created_at || 0) - new Date(left.latest_reply_at || left.sent_at || left.created_at || 0));
      const contactedThreads = threads.filter((thread) => thread.contacted || Number(thread.reply_count || 0) > 0);
      ...
      return {
        ...
        contacted_thread_count: contactedThreads.length,
        latest_contacted_outreach: contactedThreads[0] || null,
        latest_contacted_queue_row_id: contactedThreads[0]?.id || null,
