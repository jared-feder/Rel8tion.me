...
function buildAgentPerformance({
  ...
  function addUpcomingOpenHouse(profile, row, source) {
    ...
    profile.upcoming_open_houses.set(key, {
      ...
      price: row.price ?? null,
      image_url: row.image_url || row.listing_photo_url || row.image || '',
      outreach_image_url: row.outreach_image_url || row.mockup_image_url || '',
      outreach_image_status: row.outreach_image_status || row.mockup_status || '',
      outreach_image_source: row.outreach_image_url
        ? 'listing_inventory'
        : (source === 'outreach' && row.mockup_image_url ? 'outreach_queue' : ''),
      outreach_image_id: row.outreach_image_url ? row.id : (source === 'outreach' && row.mockup_image_url ? row.id : null),
      listing_url: row.listing_url || row.link || '',
      source
    });
  }
