...
const { agentNameAnchor, compatibleAgentNames } = require('./agent-name-identity');
...
function identityFrom(row = {}, prefix = '') {
  return {
    ...
    name: usableAgentName(row[`${prefix}name`]) || usableAgentName(row.agent_name) || usableAgentName(row.name) || usableAgentName(row.agent),
...
function relationshipSummary(profile) {
  ...
  const savedStatus = normalizedStatus(profile.relationship_status);
  const accepted = profile.accepted_open_house_count > 0
    || ACCEPTED_RELATIONSHIP_STATUSES.has(savedStatus)
    || [...statuses].some((status) => ACCEPTED_RELATIONSHIP_STATUSES.has(status));
  const interested = INTERESTED_RELATIONSHIP_STATUSES.has(savedStatus)
    || [...statuses].some((status) => INTERESTED_RELATIONSHIP_STATUSES.has(status));
  ...
  if (['known', 'prospect'].includes(savedStatus)) {
    return { category: 'saved_agent', label: 'Saved agent', priority: 3, has_prior_outreach: false };
  }
  return { category: 'new', label: 'New agent', priority: 4, has_prior_outreach: false };
}
...
function buildAgentPerformance({
  ...
  function ensureProfile(identity = {}, source = '') {
    identity = { ...identity, name: usableAgentName(identity.name) };
    const keysForIdentity = identityKeys(identity);
    let profile = keysForIdentity
      .map((key) => aliases.get(key))
      .find((candidate) => candidate && compatibleAgentNames(candidate.name, identity.name));
    if (!profile) {
      profile = {
    ...
    for (const field of ['agent_id', 'relationship_id', 'relationship_status', 'slug', 'ranking_id', 'name', 'phone', 'phone_normalized', 'email', 'brokerage', 'image_url', 'website', 'recommended_tier']) {
    ...
    for (const key of [...identityKeys(profile), ...keysForIdentity]) {
      const existing = aliases.get(key);
      if (!existing || compatibleAgentNames(existing.name, profile.name)) aliases.set(key, profile);
    }
    return profile;
  }
  ...
  function addUpcomingOpenHouse(profile, row, source) {
    ...
    const existing = profile.upcoming_open_houses.get(key) || {};
    const rowMedia = {
      url: row.outreach_image_url || row.mockup_image_url || '',
      status: row.outreach_image_status || row.mockup_status || '',
      source: row.outreach_image_url
        ? 'listing_inventory'
        : (source === 'outreach' && row.mockup_image_url ? 'outreach_queue' : ''),
      id: row.outreach_image_url ? row.id : (source === 'outreach' && row.mockup_image_url ? row.id : null)
    };
    const existingMedia = {
      url: existing.outreach_image_url || '',
      status: existing.outreach_image_status || '',
      source: existing.outreach_image_source || '',
      id: existing.outreach_image_id || null
    };
    const rowMediaReady = Boolean(rowMedia.url && rowMedia.status === 'rendered');
    const existingMediaReady = Boolean(existingMedia.url && existingMedia.status === 'rendered');
    const media = rowMediaReady
      ? rowMedia
      : existingMediaReady
        ? existingMedia
        : (rowMedia.url || rowMedia.status ? rowMedia : existingMedia);
    profile.upcoming_open_houses.set(key, {
      ...existing,
      id: row.open_house_id || row.id || existing.id || null,
      source_listing_id: row.source_listing_id || row.open_house_id || row.id || existing.source_listing_id || null,
      queue_row_id: row.queue_row_id || (source === 'outreach' ? row.id : null) || existing.queue_row_id || null,
      address: row.address || existing.address || '',
      open_start: row.open_start || existing.open_start,
      open_end: row.open_end || existing.open_end || null,
      price: row.price ?? existing.price ?? null,
      image_url: row.image_url || row.listing_photo_url || row.image || existing.image_url || '',
      outreach_image_url: media.url,
      outreach_image_status: media.status,
      outreach_image_source: media.source,
      outreach_image_id: media.id,
      listing_url: row.listing_url || row.link || existing.listing_url || '',
      source
    });
  }
  ...
  for (const relationship of relationships) {
    const profile = ensureProfile({
      ...
      relationship_status: relationship.relationship_status || 'known',
      slug: relationship.agent_slug || '',
      name: relationship.display_name || relationship.name || '',
      phone: relationship.phone || '',
      phone_normalized: relationship.phone_normalized || '',
      email: relationship.email || '',
      brokerage: relationship.brokerage || relationship.company || '',
      image_url: relationship.photo_url || ''
    }, 'saved agent');
    touch(profile, relationship.updated_at || relationship.created_at);
  }
  ...
  for (const row of outreach) {
    ...
    profile.outreach_threads.set(String(row.id), {
      ...
      mockup_status: row.mockup_status || ''
    });
