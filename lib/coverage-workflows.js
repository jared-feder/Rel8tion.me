...
} = require('./event-pass-registration');
...
async function createOpenHouseEvent({ sign, house, agent, source, sponsor, eventPassInventoryId = '', loanOfficerSignId = '', extraContext = {} }) {
  ...
  if (house.id && agent.slug) {
    const sharedEvent = await maybeOne(
      `open_house_events?host_agent_slug=eq.${enc(agent.slug)}&open_house_source_id=eq.${enc(house.id)}&status=eq.active&ended_at=is.null&select=*&order=created_at.desc&limit=1`
    ).catch(() => null);
    if (sharedEvent?.id) {
      const connectedSigns = await list(`smart_signs?active_event_id=eq.${enc(sharedEvent.id)}&select=id&limit=4`).catch(() => []);
      if (connectedSigns.length >= 3) {
        throw httpError(409, 'This open house already has three connected coverage devices. End or replace one before adding another.', {
...
async function activateSponsoredPass({ publicCode, body, req }) {
  ...
  const requestedAgent = mergeNonEmpty(seed?.agent || {}, body.agent || {});
  const storedAgent = await resolveExistingAgent(requestedAgent);
  const authorizationAgent = storedAgent
    ? mergeNonEmpty(requestedAgent, storedAgent)
    : requestedAgent;
  ...
  const authorization = await loadListingAuthorization({
    ...
    identityVerified: Boolean(storedAgent)
  });
  const agent = await ensureAgent(authorizationAgent, house);
  authorization.host_agent_slug = agent.slug || '';
  ...
  if (event?.id) {
    event = await attachSponsoredPassContextToEvent({
      ...
      extraContext: {
        ...
        host_authorization_verified_at: nowIso(),
