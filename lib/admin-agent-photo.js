...
function namesConflict(row, identity, spec) {
  const rowName = clean(row[spec.name], 300);
  const identityName = clean(identity.name, 300);
  return Boolean(rowName && identityName && !compatibleAgentNames(rowName, identityName));
}
...
function identityMatches(row, identity, spec) {
  ...
  if (!conflict && explicitIds.some(([wanted, actual]) => wanted && actual && String(wanted) === String(actual))) return true;

  const rowSlug = spec.slug ? cleanSlug(row[spec.slug]) : '';
  if (!conflict && identity.slug && rowSlug && identity.slug === rowSlug) return true;
