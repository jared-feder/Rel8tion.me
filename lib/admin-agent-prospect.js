...
async function resolveAgentProspectState(ranking, supabaseRest) {
  ...
  if (relationship) {
    ...
    return {
      kind: status === 'prospect' ? 'existing_prospect' : 'existing_relationship',
      label: status === 'prospect' ? 'REL8TION prospect saved' : 'Existing REL8TION relationship',
      member: null,
      relationship,
...
function nextRelationshipStatus(state) {
  ...
  return state.member ? 'member' : 'prospect';
}

async function createOrResolveAgentProspect({ ranking, supabaseRest, now = new Date() }) {
  const name = clean(ranking.agent_name || ranking.name, 300);
  const canonicalKey = canonicalRelationshipKey(ranking);
  ...
  const payload = {
    ...
    metadata: {
      ...existingMetadata,
      record_type: state.member ? 'member' : 'prospect',
      source: 'agent_ranking',
      ranking_id: ranking.id || null,
      invitation_lane: 'general_agent_invitation',
  ...
  if (!relationship?.id) throw new Error('Unable to save the REL8TION prospect record.');

  const eventType = state.member ? 'member_identified' : created ? 'prospect_created' : 'prospect_reviewed';
  await supabaseRest('agent_relationship_events', {
    ...
    body: JSON.stringify({
      ...
      summary: state.member ? 'Existing REL8TION member identified' : created ? 'REL8TION prospect created' : 'REL8TION prospect reviewed',
  ...
  const status = {
    ...
    label: state.member
      ? 'Already on REL8TION'
      : created
        ? 'REL8TION prospect created'
        : relationshipStatus === 'prospect'
          ? 'REL8TION prospect saved'
  ...
  if (ranking.id && !String(ranking.id).startsWith('relationship:')) {
    const rankingPayload = {
      ...
      raw_sources: {
        ...
        rel8tion_prospect_status: resultKind,
        rel8tion_prospect_updated_at: timestamp
      }
    };
