...
function queueOutboundMessages(queue) {
  const messages = [];
  if (queue?.selected_sms && queue?.initial_sent_at) {
    messages.push({
      id: `${queue.id}:initial`,
      ...
      from_phone: '',
      to_phone: queue.agent_phone || '',
      opt_out: false,
      message_sid: '',
      received_at: queue.initial_sent_at,
      created_at: queue.initial_sent_at,
      source: 'outreach_queue_initial'
    });
  }
  if (queue?.followup_sms && queue?.followup_sent_at) {
    messages.push({
      id: `${queue.id}:followup`,
      queue_row_id: queue.id,
      direction: 'outbound',
      ...
      opt_out: false,
      message_sid: '',
      received_at: queue.followup_sent_at,
      created_at: queue.followup_sent_at,
      source: 'outreach_queue_followup'
    });
  }
  return messages;
}
...
function mergeConversationMessages(queue, replies) {
  const rows = [...queueOutboundMessages(queue), ...(Array.isArray(replies) ? replies : [])]
    .sort((left, right) => {
      const leftAt = new Date(left?.received_at || left?.created_at || 0).getTime();
      const rightAt = new Date(right?.received_at || right?.created_at || 0).getTime();
      return leftAt - rightAt;
    });
  const deduped = [];
  for (const row of rows) {
    const time = new Date(row?.received_at || row?.created_at || 0).getTime();
    const body = comparableOutboundBody(row?.body);
    const duplicate = deduped.find((candidate) => {
      if ((candidate?.direction || 'inbound') !== (row?.direction || 'inbound')) return false;
      if (comparableOutboundBody(candidate?.body) !== body) return false;
      const candidateTime = new Date(candidate?.received_at || candidate?.created_at || 0).getTime();
      return Number.isFinite(time) && Number.isFinite(candidateTime) && Math.abs(time - candidateTime) <= 5 * 60 * 1000;
    });
...
function serializeThread(row) {
  const optedOut = isOptedOut(row);
  const sentQueueMessageCount = Number(Boolean(row.selected_sms && row.initial_sent_at))
    + Number(Boolean(row.followup_sms && row.followup_sent_at));
  const phoneNormalized = row.agent_phone_normalized || row.from_phone_normalized || '';
  const identityKey = phoneNormalized
    ? `phone:${phoneNormalized}`
    : row.agent_email
      ? `email:${String(row.agent_email).trim().toLowerCase()}`
      : `name:${String(row.agent_name || '').trim().toLowerCase()}|${String(row.brokerage || '').trim().toLowerCase()}`;
  return {
    ...
    message_count: Number(row.message_count ?? (Number(row.reply_count || 0) + sentQueueMessageCount)),
    latest_message: {
...
async function listThreads(options = {}) {
  ...
  const [inboundRows, recentRows, queueRows] = await Promise.all([
    supabaseRest(
    ...
    supabaseRest(
      `agent_outreach_queue?select=${QUEUE_SELECT}&order=last_outreach_at.desc.nullslast,created_at.desc&limit=${fetchLimit}`
    )
  ]);

  const queueThreadRows = (Array.isArray(queueRows) ? queueRows : []).map((row) => ({
    ...row,
    queue_row_id: row.id,
    last_reply_at: row.followup_sent_at || row.initial_sent_at || row.last_outreach_at || row.created_at || null,
    latest_reply_body: row.followup_sent_at ? row.followup_sms : row.selected_sms || '',
    latest_reply_opt_out: false,
    direction: 'outbound',
    reply_count: 0
  }));
  const merged = mergeInboxRows(inboundRows, recentRows, queueThreadRows)
    .filter((row) => !cursorDate || rowTime(row) < cursorDate.getTime())
    .filter((row) => matchesFilter(row, filter));
  const pageRows = merged.slice(0, limit);
  const queueMap = new Map((Array.isArray(queueRows) ? queueRows : []).map((row) => [row.id, row]));
  const missingQueueIds = pageRows
    .map((row) => row.queue_row_id)
    .filter((id) => id && !queueMap.has(id));
  const missingQueueMap = await loadQueueRows(missingQueueIds);
  for (const [id, row] of missingQueueMap) queueMap.set(id, row);
  ...
  const nextCursor = merged.length > limit && pageRows.length
    ? new Date(rowTime(pageRows[pageRows.length - 1])).toISOString()
    : null;
...
async function getThread(threadId) {
  ...
  const participants = visit?.id
    ...
    : [];
  const messageRows = mergeConversationMessages(queue, messages);
