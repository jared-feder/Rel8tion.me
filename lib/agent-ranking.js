const FIELD_SYNONYMS = {
  ...
  brokerage: ['brokerage', 'company', 'office', 'brokerage_name', 'firm', 'agency'],
  ...
  active_listing_count: ['active_listings', 'active_listing_count', 'listings', 'active_units'],
  ...
  market_area: ['market_area', 'market', 'area', 'region'],
  city: ['city', 'town'],
  county: ['county'],
  state: ['state', 'st']
};
...
function normalizeImportRows(csvText, options = {}) {
  ...
  for (const rawRow of rows.slice(1)) {
    ...
    const normalized = {
      ...
      email,
      market_area: valueFor(rawRow, mapping, 'market_area') || cleanText(options.market_area),
      city: valueFor(rawRow, mapping, 'city'),
      county: valueFor(rawRow, mapping, 'county'),
      state: valueFor(rawRow, mapping, 'state'),
      production_volume: Math.max(0, toNumber(valueFor(rawRow, mapping, 'production_volume'))),
      transaction_count: toInt(valueFor(rawRow, mapping, 'transaction_count')),
...
function tierForScore(score, row) {
  if (!row.agent_name && !row.production_volume && !row.transaction_count) return 'Unknown';
...
function nextBestAction(row, tier) {
  if (tier === 'A+') return 'Prioritize owner-led outreach with production-focused kit pitch.';
  if (tier === 'A') return 'Send production-focused open house kit intro.';
...
function scoreRow(row, averages = {}) {
  ...
  const productionScore =
    ...
    (Number(row.active_listing_count || 0) * 15) +
  ...
  const openHouseBoost = row.has_open_house_this_weekend ? 35 : openHouseCount > 0 ? 18 : 0;
  const gapScore =
    (highProduction ? 40 : 0) +
    (highTransactions ? 25 : 0) +
    (strongPrice ? 20 : 0) +
    (Number(row.active_listing_count || 0) > 0 ? 15 : 0) +
    openHouseBoost +
    (row.phone_normalized || row.email ? 10 : 0);
  const captureScore = Math.max(0, Math.min(100, 100 - gapScore + (openHouseCount ? 10 : 0)));
  const score = Math.round(productionScore + gapScore + contactScore);
  ...
  return {
    ...
    labels: [
      ...
      Number(row.active_listing_count || 0) > 0 && !openHouseCount ? 'Underutilized Producer' : '',
...
function outreachPayloadFromRanking(ranking) {
  ...
  return {
    ...
    report_note: [
      ...
      `Production: ${money(ranking.production_volume)} volume, ${ranking.transaction_count || 0} transactions.`,
      ranking.gap_summary || '',
