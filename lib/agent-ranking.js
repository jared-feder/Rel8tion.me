...
function identityLocation(row = {}) {
  return normalizeIdentityPart(row.primary_county || row.county || row.market_area || row.city || row.state || '');
}
...
function dedupeRowsByIdentityKey(rows = []) {
  ...
  for (const row of rows || []) {
    const identityKey = row.identity_key || identityKeyForAgentRanking(row);
    if (!identityKey) {
...
function activityProofPoints(row = {}) {
  ...
  if (activeListings > 0) points.push(`${activeListings} active ${plural(activeListings, 'listing')}`);
...
function captureGapSentence(row = {}) {
  ...
  if (activeListings > 0 && matchedOpenHouses > 0) {
    return `You have ${matchedOpenHouses} visible Rel8tion open-house ${plural(matchedOpenHouses, 'match', 'matches')} against ${activeListings} active ${plural(activeListings, 'listing')}, so the upside is making more of that listing traffic visible.`;
  }
  if (activeListings > 0) {
    return `With ${activeListings} active ${plural(activeListings, 'listing')}, the missed opportunity is the buyer traffic you already created but cannot see or follow up with instantly.`;
  }
  const listingSide12 = Number(row.listings_active_last_12_months || 0);
  if (listingSide12 > 0) {
...
function rankingFromImportRow(importRow, averages = {}, signals = {}) {
  ...
  const ranking = {
    ...
    identity_key: importRow.identity_key || identityKeyForAgentRanking(importRow) || null,
