const { inferCountyFromRow, normalizeCounty } = require('./location-intelligence');
...
const FIELD_SYNONYMS = {
  ...
  buyside_last_90_days: ['buyside_last_90_days', 'buyside_sales_last_90_days', 'buyer_side_last_90_days', 'buy_side_last_90_days', 'buyer_sides_90_days'],
  buyside_last_12_months: ['buyside_last_12_months', 'buyside_sales_last_12_months', 'buyer_side_last_12_months', 'buy_side_last_12_months', 'buyer_sides_12_months'],
...
function identityKeyForAgentRanking(row = {}) {
  const agentName = normalizeIdentityPart(row.agent_name || [row.first_name, row.last_name].filter(Boolean).join(' '));
  const phone = normalizePhone(row.phone_normalized || row.phone);
  if (!agentName || !phone) return '';
  return `import:${agentName}|${phone}`;
}

const LISTREPORTS_ACTIVITY_FIELDS = [
...
function listReportsActivityTotal(row = {}) {
  return LISTREPORTS_ACTIVITY_FIELDS.reduce((sum, field) => sum + Number(row[field] || 0), 0);
}

function assessListReportsChange(previous = {}, candidate = {}) {
  const deltas = Object.fromEntries(LISTREPORTS_ACTIVITY_FIELDS.map((field) => [
    field,
    Number(candidate[field] || 0) - Number(previous[field] || 0)
  ]));
  const previousTotal = listReportsActivityTotal(previous);
  const candidateTotal = listReportsActivityTotal(candidate);
  const totalDelta = candidateTotal - previousTotal;
  const severeDrop = previousTotal >= 15
    && totalDelta <= -15
    && candidateTotal <= previousTotal * 0.25;
  const largeIncrease = totalDelta >= 15
    && candidateTotal >= Math.max(previousTotal + 15, previousTotal * 1.75);
  ...
  return {
    ...
    deltas,
    listings_days_since_last_delta: Number(candidate.listings_days_since_last || 0)
      - Number(previous.listings_days_since_last || 0),
    severe_drop: severeDrop,
    large_increase: largeIncrease,
    classification: severeDrop ? 'severe_drop' : largeIncrease ? 'large_increase' : 'normal'
  };
}
...
function rowIdentity(row) {
  return identityKeyForAgentRanking(row);
}

function identityStrength(row = {}) {
  return [
    ...
    Number(row.match_confidence || row.raw_sources?.match_confidence || 0)
  ];
}

function strongerIdentityRow(left, right) {
  const a = identityStrength(left);
  const b = identityStrength(right);
...
function strongerListReportsSnapshot(left, right) {
  const fields = [
    'active_listing_count',
    'listings_active_last_12_months',
    'buyside_last_12_months',
  ...
  const leftDays = Number(left?.listings_days_since_last || 0);
  const rightDays = Number(right?.listings_days_since_last || 0);
  if (leftDays !== rightDays) {
    if (!leftDays) return right;
    if (!rightDays) return left;
    return leftDays < rightDays ? left : right;
  }
  return left;
}

function dedupeRowsByIdentityKey(rows = []) {
  const map = new Map();
  const duplicates = [];
  const skippedMissingIdentity = [];

  for (const row of rows || []) {
    const identityKey = identityKeyForAgentRanking(row) || row.identity_key;
    if (!identityKey) {
      skippedMissingIdentity.push({
        ...row,
        identity_missing_reason: !normalizeIdentityPart(row.agent_name || [row.first_name, row.last_name].filter(Boolean).join(' '))
          ? 'missing_agent_name'
          : 'missing_phone'
      });
      continue;
    ...
    const keyed = { ...row, identity_key: identityKey };
    const existing = map.get(identityKey);
    if (!existing) {
      map.set(identityKey, keyed);
      continue;
    }

    const strongest = strongerIdentityRow(existing, keyed);
    duplicates.push(strongest === existing ? keyed : existing);
    map.set(identityKey, strongest);
  }
  ...
  return {
    ...
    skipped_missing_identity_count: skippedMissingIdentity.length
  };
}
...
function normalizeImportRows(csvText, options = {}) {
  ...
  for (const rawRow of rows.slice(1)) {
    ...
    const explicitTransactions = mapping.transaction_count
      ? toInt(valueFor(rawRow, mapping, 'transaction_count'))
      : 0;
    ...
    const normalized = {
      ...
      transaction_count: explicitTransactions,
    ...
    const identity = identityKeyForAgentRanking(normalized);
    normalized.identity_key = identity;
    normalized.identity_missing_reason = identity
      ? ''
      : !normalizeIdentityPart(normalized.agent_name)
        ? 'missing_agent_name'
        : 'missing_phone';
    normalized.duplicate_key = identity;
    normalized.is_duplicate = Boolean(identity && seen.has(identity));
    if (identity) seen.set(identity, (seen.get(identity) || 0) + 1);
    parsed.push(normalized);
  }

  const duplicates = parsed.filter((row) => row.duplicate_key && seen.get(row.duplicate_key) > 1).length;
  return {
    headers,
    normalized_headers,
    mapping,
...
function matchImportedRows(rows, agents = []) {
  ...
  for (const agent of agents || []) {
    ...
    add(byPhone, phone, agent);
    add(byEmail, email, agent);
  }
  ...
  return rows.map((row) => {
    ...
    if (row.phone_normalized && byPhone.has(row.phone_normalized)) {
      const candidates = byPhone.get(row.phone_normalized);
      const compatible = candidates.filter((agent) => compatibleAgentNames(row.agent_name, agent.name));
      matched = compatible.length === 1 ? compatible[0] : candidates[0];
      if (compatible.length === 1) {
        confidence = 100;
        reason = 'phone';
      } else {
        confidence = 50;
        reason = compatible.length > 1 ? 'phone_identity_ambiguous' : 'phone_name_conflict';
      }
      ...
      if (compatible.length === 1) {
        ...
        confidence = 50;
        reason = compatible.length > 1 ? 'email_identity_ambiguous' : 'email_name_conflict';
      }
    } else {
    ...
    const trustedMatch = Boolean(matched && confidence >= 75);
    return {
      ...row,
      matched_agent_id: trustedMatch ? matched.id : null,
...
function safeLocationPart(value) {
  const text = cleanText(value);
  if (!text || looksLikeEncodedGeometry(text)) return '';
  return text;
}

function safeMarketArea(row = {}) {
  const county = normalizeCounty(row.primary_county || row.county || '');
  const market = safeLocationPart(row.market_area);
  if (market) {
    const normalized = market.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
    if (['lng island', 'long island', 'li', 'nassau suffolk', 'nassau and suffolk'].includes(normalized)) return 'Long Island';
    if (normalized === 'nassau county') return 'Nassau';
    if (normalized === 'suffolk county') return 'Suffolk';
    if (!county || normalizeName(market) !== normalizeName(county)) return market;
  }
  return county || safeLocationPart(row.city) || safeLocationPart(row.state);
}
...
function activityProofPoints(row = {}) {
  ...
  if (activeListings > 0) points.push(`a ListReports listing signal of ${activeListings}`);
  if (listingSide12 > 0) points.push(`${listingSide12} listing-side ${plural(listingSide12, 'deal')} in 12 months`);
  if (buySide90 > 0) points.push(`${buySide90} buyside ${plural(buySide90, 'deal')} in 90 days`);
  if (buySide12 > 0) points.push(`${buySide12} buyside ${plural(buySide12, 'deal')} in 12 months`);
  if (daysSince > 0) points.push(`a listing ${daysSince} ${plural(daysSince, 'day')} ago`);
  if (matchedWeekend > 0) points.push(`${matchedWeekend} matched weekend open ${plural(matchedWeekend, 'house')}`);
  else if (matchedOpenHouses > 0) points.push(`${matchedOpenHouses} matched open-house ${plural(matchedOpenHouses, 'record')}`);

  return points;
}

function activityProofSentence(row = {}, { max = 3 } = {}) {
  const points = activityProofPoints(row).slice(0, max);
  if (!points.length) return 'your ListReports production activity';
  if (points.length === 1) return points[0];
  if (points.length === 2) return `${points[0]} and ${points[1]}`;
  return `${points.slice(0, -1).join(', ')}, and ${points[points.length - 1]}`;
}

function captureGapSentence(row = {}) {
  const activeListings = Number(row.active_listing_count || 0);
  const matchedOpenHouses = Number(row.matched_open_house_count || row.open_house_count || 0);
  if (activeListings > 0 && matchedOpenHouses > 0) {
    return `You have ${matchedOpenHouses} visible Rel8tion open-house ${plural(matchedOpenHouses, 'match', 'matches')}, while ListReports imported a listing signal of ${activeListings}. REL8TION verifies current inventory separately, so the upside is making more real listing traffic visible.`;
  }
  if (activeListings > 0) {
    return `ListReports imported a listing signal of ${activeListings}; that is prospecting intelligence, not verified current inventory. The opportunity is capturing the buyer traffic around the agent's next verified listing or open house.`;
  }
  const listingSide12 = Number(row.listings_active_last_12_months || 0);
  if (listingSide12 > 0) {
    return `With ${listingSide12} listing-side ${plural(listingSide12, 'deal')} in 12 months, the value is capturing the buyers around that listing activity instead of letting them disappear after a showing or open house.`;
  }
  return 'Rel8tion turns the next listing or open house into a visible buyer-capture moment without making you learn another platform.';
}
...
function buildGapSummary(row, averages) {
  ...
  if (Number(row.matched_open_house_count || 0) > 0) {
    return 'This agent has a current matched REL8TION open house, so outreach can focus on capturing buyers from that verified event.';
  }
  const activeListings = Number(row.active_listing_count || 0);
  const listingSide12 = Number(row.listings_active_last_12_months || 0);
  const buySide12 = Number(row.buyside_last_12_months || 0);
  ...
  if (activeListings > 0 && daysSince > 0 && daysSince <= 45) {
    return 'ListReports imported a recent listing-activity signal for this agent. Verify current inventory in REL8TION before treating it as a live listing or open-house opportunity.';
  }
  ...
  if (row.active_listing_count > 0 || row.open_house_count > 0) {
    return 'ListReports shows listing activity, but current inventory must be verified in REL8TION before outreach is tied to a specific listing or open house.';
  }
...
function buildPitch(row) {
  ...
  const market = safeMarketArea(row);
  const marketText = market ? ` in ${market}` : '';
  const proof = activityProofSentence(row, { max: 3 });
  return `${first}, I saw ${proof}${marketText}. ${captureGapSentence(row)} Rel8tion gives you a branded tap-or-scan open-house kit that captures buyers, handles disclosures, sends instant follow-up, and keeps you from managing another app or password.`;
}
...
function nextBestAction(row, tier) {
  ...
  if (tier === 'A') return 'Verify the next listing, then send an open-house kit intro.';
...
function cappedScore(value, cap, weight) {
  const number = Number(value || 0);
  if (!Number.isFinite(number) || number <= 0) return 0;
  return Math.min(number, cap) * weight;
}
...
function scoreRow(row, averages = {}) {
  ...
  const productionScore =
    cappedScore(row.transaction_count, 60, 3) +
    cappedScore(Number(row.production_volume || 0) / 100000, 80, 1) +
    cappedScore(activeListings, 30, 5) +
    cappedScore(listingSide12, 120, 1.6) +
    cappedScore(buySide90, 60, 2) +
    cappedScore(buySide12, 120, 1) +
    freshListingScore +
    cappedScore(row.sold_listing_count, 60, 2) +
    cappedScore(Number(row.average_price || 0) / 100000, 80, 0.8) +
    cappedScore(openHouseCount, 10, 12);
  const rel8tionActivityScore =
    cappedScore(matchedActiveListings, 10, 8) +
    cappedScore(matchedOpenHouses, 10, 12) +
    cappedScore(matchedWeekend, 5, 24);
  ...
  return {
    ...
    labels: [
      ...
      activeListings > 0 ? 'ListReports Listing Signal' : '',
...
function rankingFromImportRow(importRow, averages = {}, signals = {}) {
  ...
  const ranking = {
    ...
    identity_key: identityKeyForAgentRanking(importRow) || importRow.identity_key || null,
  ...
  if (!ranking.identity_key) ranking.raw_sources.identity_missing_reason = importRow.identity_missing_reason || 'missing_phone';
  return ranking;
}
...
function buildPitchVariants(ranking) {
  ...
  const marketName = safeMarketArea(ranking);
  const market = marketName ? ` in ${marketName}` : '';
  const proof = activityProofSentence(ranking, { max: 3 });
  const proofShort = activityProofSentence(ranking, { max: 2 });
  ...
  const matchedOpenHouses = Number(ranking.matched_open_house_count || ranking.open_house_count || 0);
  const teamSignal = activeListings > 0
    ? `a ListReports listing signal of ${activeListings}`
    : listingSide12 > 0
      ? `${listingSide12} listing-side ${plural(listingSide12, 'deal')} in 12 months`
      : '';
  const openHouseLine = matchedOpenHouses > 0
    ? `${matchedOpenHouses} matched open-house ${plural(matchedOpenHouses, 'record')} already ${plural(matchedOpenHouses, 'shows', 'show')} where Rel8tion can plug in.`
    : 'The next open house can become a clean buyer-capture moment instead of another paper sign-in or forgotten follow-up.';
  return {
    soft_intro: `Hi ${first}, saw ${proof}${market}. Rel8tion is a no-setup open-house kit: buyers tap or scan, check in, disclosures are handled, and you get instant lead visibility. No app or password for you to manage.`,
    production_focused: `${first}, ${proofShort} tells me you are already creating real buyer traffic${brokerage}. Rel8tion helps capture the buyers your listings already attract with a branded smart sign, instant SMS, disclosures, and follow-up without adding another platform to learn.`,
    open_house_kit_focused: `For your next open house, Rel8tion can give you a physical branded kit that is already live. ${openHouseLine} Buyers tap or scan, you get the lead, and the system handles the recap and follow-up.`,
    broker_team_focused: `${first}, ${teamSignal ? `with ${teamSignal}, ` : ''}Rel8tion gives busy agents and teams a repeatable buyer-capture workflow without asking anyone to print QR codes, remember links, create logins, or learn another dashboard.`,
    luxury_agent_focused: `${first}, when your profile shows ${proofShort}, every missed open-house buyer matters. Rel8tion keeps the experience polished: branded hardware, instant buyer capture, disclosures, and financing support when needed.`
  };
}
...
function outreachPayloadFromRanking(ranking) {
  ...
  return {
    ...
    report_note: [
      ...
      `ListReports activity: imported listing signal ${ranking.active_listing_count || 0} (not verified current inventory), ${ranking.listings_active_last_12_months || 0} listing-side 12m, ${ranking.buyside_last_90_days || 0} buyside 90d, ${ranking.buyside_last_12_months || 0} buyside 12m.`,
      `Location: ${ranking.primary_county || ranking.county || 'Needs review'}${safeMarketArea(ranking) ? ` / ${safeMarketArea(ranking)}` : ''}. Matched open houses: ${ranking.matched_open_house_count || 0}, weekend: ${ranking.matched_weekend_open_house_count || 0}.`,
      ranking.gap_summary || '',
