...
function recordMatchesRanking(ranking = {}, record = {}) {
  if (!compatibleAgentNames(ranking.agent_name || ranking.name, record.agent_name || record.name)) return false;
...
function matchingAgentSlugs(ranking = {}, agents = []) {
  ...
  for (const agent of agents || []) {
    const idMatch = compatibleAgentNames(ranking.agent_name, agent.name)
      && ranking.agent_id && agent.id && String(ranking.agent_id) === String(agent.id);
