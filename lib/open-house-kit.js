...
function publicOrder(order = {}) {
  ...
  const websiteBuilderIncluded = ['website_included', 'website_builder_included']
    .some((key) => String(metadata[key] || '').toLowerCase() === 'true');
  ...
  return {
    ...
    plan_code: clean(metadata.plan_code, 100) || null,
    entitlement_codes: clean(metadata.entitlement_codes, 500).split(',').filter(Boolean),
    branded_rel8tionchips: String(metadata.branded_rel8tionchips || '').toLowerCase() === 'true',
    branding_status: clean(metadata.branding_status, 40) || null,
    brokerage_name: clean(metadata.brokerage_name || metadata.brokerage, 180) || null,
    brand_logo_url: clean(metadata.brand_logo_url, 500) || null,
    preferred_company_colors: clean(metadata.preferred_company_colors, 300) || null,
    brand_notes: clean(metadata.brand_notes, 500) || null,
    digital_card_included: String(metadata.digital_card_included || '').toLowerCase() === 'true',
    content_tools_included: String(metadata.content_tools_included || '').toLowerCase() === 'true',
