...
- `[IMPLEMENTED]` SMS provider selection supports `SMS_OUTREACH_PROVIDER` for outreach/manual outreach and `SMS_EVENTS_PROVIDER` for buyer/event/owner operational traffic, both falling back to `SMS_PROVIDER`. Production is configured with `SMS_OUTREACH_PROVIDER=twilio`, toll-free outreach sender `+18448211802` through Messaging Service `MG8d7ec49cf1d6d231080b7f870a10eb0b`, and `SMS_EVENTS_PROVIDER=twilio` with regular event sender `+15168885461`. Toll-free registration, outbound MMS, inbound reply storage, queue linking, and the owner-alert route are verified. As of 2026-07-22, automatic outreach is hard-capped at 5/run, 20/hour, and 150 per rolling 24 hours, with a 1% seven-day opt-out health gate; do not release old manual backlog rows.
...
- `[IMPLEMENTED]` Automatic outreach is cron-driven and must not require hidden per-row approval when enabled. Eligible rows are `send_mode=automatic`, generated, rendered, due, with a listing photo and pending SMS copy. As of 2026-07-22, the owner placed outreach in Live/manual mode and moved all unsent future automatic rows to `send_mode=manual`; do not return them to automatic without explicit owner direction.
- `[IMPLEMENTED]` The current owner-approved production ceilings are 5 per run, 20 per rolling hour, and 150 per rolling 24 hours. The seven-day opt-out health gate activates at 1% after at least 20 logged outreach sends. Do not raise these ceilings or the health threshold without explicit owner approval and provider health review.
...
- `[IMPLEMENTED]` REL8TION COMMAND's Event Pass Freshener is admin-only and Event-Pass-only. It may unclaim a `keys` row only when the backing sign is positively identified as an Event Pass and the key has `device_role=event_pass_keychain`; never use it to unclaim a normal agent keychain or generic Smart Sign chip. Preserve history/check-ins/disclosures, printed codes, sponsor configuration, and reuse settings.
