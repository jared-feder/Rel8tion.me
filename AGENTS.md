...
- `[IMPLEMENTED]` REL8TION COMMAND's Signs tab renders only normal Event Passes that have an explicit inventory assignment, a claimed NFC owner, or a live event. Live passes sort first, followed by claimed and explicitly assigned passes with the most recently activated first. Keep fresh unassigned inventory available to the scanner and QR-batch workflows without rendering hundreds of unused rows in the operator table.
